...
impl Surface {
    ...
    pub fn render_lines(&self, color: bool) -> String {
        if !color {
            let mut out = String::new();
            for y in 0..self.height {
        ...
        for y in 0..self.height {
            ...
            for x in 0..self.width {
                let c = &self.back[y * self.width + x];
                if c.is_cont { continue; }
                let blank = c.ch == ' ' && c.fg == Col::DEFAULT && c.bg == Col::DEFAULT && c.style == Style::NONE;
                if !blank { last = Some(x); }
            }
            if let Some(last) = last {
                ...
                for x in 0..=last {
                    let c = self.back[y * self.width + x];
                    if c.is_cont { continue; }
                    if !init || c.fg != cur_fg || c.bg != cur_bg || c.style != cur_st {
                        out.push_str("\x1b[0");
                        if c.style.contains(Style::BOLD) { out.push_str(";1"); }
                        if c.style.contains(Style::ITALIC) { out.push_str(";3"); }
                        if c.style.contains(Style::UNDERLINE) { out.push_str(";4"); }
                        if c.style.contains(Style::REVERSE) { out.push_str(";7"); }
                        append_fg(&mut out, c.fg);
                        if !c.bg.is_default() { append_bg(&mut out, c.bg); }
                        out.push('m');
                        cur_fg = c.fg;
                        cur_bg = c.bg;
                        cur_st = c.style;
...
mod tests {
    ...
    #[test]
    fn render_lines_plain_trims_and_no_ansi() {
        ...
        let plain = s.render_lines(false);
        assert_eq!(plain, "Hello\nworld");
        assert!(!plain.contains('\x1b'));
    }

    #[test]
    fn render_lines_color_emits_sgr_and_resets() {
        let mut s = Surface::new(20, 2);
        s.write_str(0, 0, "Hi", Col::named(2), Col::DEFAULT, Style::BOLD); 
        s.write_str(0, 1, "x", Col::DEFAULT, Col::DEFAULT, Style::NONE);   
        let c = s.render_lines(true);
        assert!(c.contains("\x1b[0;1;32mHi"), "bold(1) green(32) prefix: {c:?}");
        assert!(c.contains("Hi\x1b[0m"), "reset at line end");
        
        assert!(c.contains("\x1b[0mx\x1b[0m"), "default line is clean: {c:?}");
        assert!(!c.contains(";48;2;0;0;0"), "default bg is NOT forced to black in snapshots");
    }
