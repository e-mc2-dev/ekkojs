...
pub(crate) fn drain_snapshot_frames(
    ...
    let mut got = false;
    let hard = Instant::now() + Duration::from_millis(hard_ms);
    ...
    loop {
        ...
        let timeout = grace
            .map(|g| g.saturating_duration_since(Instant::now()))
            .unwrap_or(Duration::from_millis(hard_ms));
        match cmd_rx.recv_timeout(timeout) {
            Ok(TuiCommand::UpdateCells(json)) => {
                apply_cell_updates(&mut surface, &json);
                got = true;
                grace = Some(Instant::now() + Duration::from_millis(grace_ms)); 
            }
...
pub fn run_snapshot(
    ...
    let (cols, rows) = ekko_core::bridges::tui_bridge::get_size();
    let (surface, got) = drain_snapshot_frames(&cmd_rx, cols as usize, rows as usize, 2000, 120);

    use std::io::IsTerminal;
    let color = std::env::var_os("NO_COLOR").is_none() && std::io::stdout().is_terminal();
    println!("{}", surface.render_lines(color));
    if !got {
        eprintln!("[snapshot] no frame produced within 2s — did the app call render(<App/>)?");
    }

    drop(event_tx);

    let _ = std::io::stdout().flush();
    std::process::exit(if got { 0 } else { 1 });
}
    ...
    loop {
        ...
        if let Some(event) = input::poll_event(Duration::from_millis(16)) {
            ...
            if let TuiEvent::Key(ref k) = event {
                if k.ctrl && (k.ch == '\x03' || k.ch == 'c' || k.ch == '\x11' || k.ch == 'q') {
                    restore_and_banner(&mut terminal, &mut stdout);
                    std::process::exit(0);
                }
...
mod tests {
    ...
    fn cells(s: &str, y: usize) -> String {
        let mut v = Vec::new();
        for (x, ch) in s.chars().enumerate() {
            v.push(format!(r#"{{"x":{},"y":{},"ch":"{}"}}"#, x, y, ch));
        }
        format!("[{}]", v.join(","))
    }

    #[test]
    fn snapshot_captures_first_frame() {
        let (tx, rx) = mpsc::channel();
        tx.send(TuiCommand::UpdateCells(cells("HI", 0))).unwrap();
        drop(tx); 
        let (surface, got) = drain_snapshot_frames(&rx, 10, 2, 2000, 120);
        assert!(got, "a frame was sent");
        assert!(surface.render_lines(false).starts_with("HI"));
    }

    #[test]
    fn snapshot_no_frame_times_out_without_hanging() {
        let (_tx, rx) = mpsc::channel::<TuiCommand>(); 
        let start = std::time::Instant::now();
        let (_surface, got) = drain_snapshot_frames(&rx, 8, 4, 40, 10); 
        assert!(!got, "no frame was ever sent");
        assert!(start.elapsed() < Duration::from_millis(500), "must not hang past the hard ceiling");
    }

    #[test]
    fn snapshot_merges_synchronous_re_renders_within_grace() {
        let (tx, rx) = mpsc::channel();
        
        tx.send(TuiCommand::UpdateCells(cells("AB", 0))).unwrap();
        tx.send(TuiCommand::UpdateCells(cells("CD", 1))).unwrap();
        drop(tx);
        let (surface, got) = drain_snapshot_frames(&rx, 10, 3, 2000, 120);
        assert!(got);
        let out = surface.render_lines(false);
        assert!(out.contains("AB") && out.contains("CD"), "both frames merged: {out:?}");
    }

    #[test]
    fn snapshot_stops_on_quit() {
        let (tx, rx) = mpsc::channel();
        tx.send(TuiCommand::UpdateCells(cells("X", 0))).unwrap();
        tx.send(TuiCommand::Quit).unwrap();
        tx.send(TuiCommand::UpdateCells(cells("Y", 1))).unwrap(); 
        let (surface, got) = drain_snapshot_frames(&rx, 10, 3, 2000, 120);
        assert!(got);
        let out = surface.render_lines(false);
        assert!(out.contains("X") && !out.contains("Y"), "Quit ends collection: {out:?}");
    }
