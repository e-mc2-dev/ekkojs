...
fn snapshot_size() -> (u16, u16) {
    let cols = std::env::var("COLUMNS").ok().and_then(|s| s.parse().ok()).unwrap_or(100);
    let rows = std::env::var("LINES").ok().and_then(|s| s.parse().ok()).unwrap_or(40);
    (cols, rows)
}

pub fn run(file: PathBuf, title: String, snapshot: bool) -> anyhow::Result<()> {
    launch(file, title, None, snapshot)
}

pub fn run_dev(file: PathBuf, title: String, project_dir: PathBuf) -> anyhow::Result<()> {
    launch(file, title, Some(project_dir), false)
}

pub fn run_from_vfs(code: String, filename: String, _title: String, snapshot: bool) -> anyhow::Result<()> {
    ...
    if snapshot {
        let (c, r) = snapshot_size();
        ekko_core::bridges::tui_bridge::init_size(c, r);
    }
    ...
    if snapshot {
        bridge::run_snapshot(cmd_rx, event_tx)
    } else {
        bridge::run_event_loop(cmd_rx, event_tx)
    }
}

fn launch(file: PathBuf, _title: String, watch_dir: Option<PathBuf>, snapshot: bool) -> anyhow::Result<()> {
