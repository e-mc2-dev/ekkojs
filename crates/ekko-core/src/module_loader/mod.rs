...
fn resolve_builtin<'a>(
    ...
    let result = match module_name {
        ...
        "jsx-runtime" => {
            let names: Vec<&str> = vec!["jsx", "jsxs", "Fragment", "registerReact"];
            let exports: Vec<v8::Local<v8::String>> = names.iter()
                .map(|n| v8::String::new(scope, n).unwrap()).collect();
        ...
        "rune/router" => {
            let names: Vec<&str> = vec!["createRouter", "useRouter", "useParams", "useSearchParams", "navigate", "validateUrl", "matchPath", "extractParams"];
            let exports: Vec<v8::Local<v8::String>> = names.iter()
                .map(|n| v8::String::new(scope, n).unwrap()).collect();
