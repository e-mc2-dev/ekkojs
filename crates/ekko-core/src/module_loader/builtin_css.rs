...
fn css_css_modules(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue) {
    let input = args.get(0).to_rust_string_lossy(scope);
    let filename = if args.length() > 1 && !args.get(1).is_null_or_undefined() { args.get(1).to_rust_string_lossy(scope) } else { "module.css".to_string() };
    let mut pattern: Option<String> = None;
    if args.length() > 2 && args.get(2).is_object() {
        let opts = v8::Local::<v8::Object>::try_from(args.get(2)).unwrap();
        let pk = v8::String::new(scope, "pattern").unwrap();
        if let Some(pv) = opts.get(scope, pk.into()) {
            if pv.is_string() { pattern = Some(pv.to_rust_string_lossy(scope)); }
        }
    }
    match crate::parsers::css::extract_css_modules(&input, &filename, pattern.as_deref()) {
        Ok(result) => {
            let obj = v8::Object::new(scope);
            let ck = v8::String::new(scope, "code").unwrap();
            let cv = v8::String::new(scope, &result.code).unwrap_or_else(|| v8::String::empty(scope));
