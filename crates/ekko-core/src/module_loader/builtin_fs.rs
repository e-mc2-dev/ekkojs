...
use crate::engine::v8_runtime::{resolve_read, resolve_write, read_permit, vfs_meta, vfs_children, ReadTarget, VfsMeta};
...
fn fs_read_dir(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue) {
    ...
    let json = match vfs_meta(&path) {
        Some(VfsMeta::File(_)) => { fs_throw!(scope, format!("ENOTDIR: '{}' is a file", path)); return; }
        Some(VfsMeta::Dir) => {
            let items: Vec<String> = vfs_children(&path).unwrap_or_default().iter()
                .map(|(n, is_dir)| format!(r#"{{"name":"{}","isFile":{},"isDirectory":{}}}"#, n, !is_dir, is_dir))
                .collect();
            format!("[{}]", items.join(","))
        }
        _ => match resolve_read(&path) {
            ReadTarget::Vfs(_) => "[]".to_string(),
            ReadTarget::Disk(p) => {
                if !read_permit(&p) { throw_read_denied(scope, &p); return; }
                match std::fs::read_dir(&p) {
                    Ok(entries) => {
                        let items: Vec<String> = entries.flatten().map(|e| {
                            let name = e.file_name().to_string_lossy().to_string();
                            let is_dir = e.file_type().map(|t| t.is_dir()).unwrap_or(false);
                            format!(r#"{{"name":"{}","isFile":{},"isDirectory":{}}}"#, name, !is_dir, is_dir)
                        }).collect();
                        format!("[{}]", items.join(","))
                    }
                    Err(e) => { fs_throw!(scope, format!("{}: {}", errno_prefix(&e), e)); return; }
