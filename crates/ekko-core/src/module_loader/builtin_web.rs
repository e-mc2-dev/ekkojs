...
pub fn web_read_body_chunk(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue) {
    let handle = args.get(0).int32_value(scope).unwrap_or(0);
    
    let api = match scope.get_slot::<Arc<crate::ffi::generated::web_server_api::Api>>().cloned() {
        Some(a) => a,
        None => { fs_throw!(scope, "web: .NET library not loaded"); return; }
    };
    ...
    tokio::spawn(async move {
        let result = api.webserver_readBodyChunk(handle).await;
        let completion = match result {
            Ok(bytes) => Ok(format!("[{}]", bytes.iter().map(|b| b.to_string()).collect::<Vec<_>>().join(","))),
            Err(e) => Err(e),
        };
        let _ = tx.send(crate::ops::async_state::AsyncCompletion { id, result: completion }).await;
        notify.notify_one();
