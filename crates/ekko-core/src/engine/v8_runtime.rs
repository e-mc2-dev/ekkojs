...
pub fn init_v8() {
    V8_INIT.call_once(|| {
        ...
        #[repr(align(16))]
        struct Align16<T: ?Sized>(T);
        static ICU_DATA: &Align16<[u8]> = &Align16(*include_bytes!("icudtl.dat"));

        
        if let Err(code) = v8::icu::set_common_data_74(&ICU_DATA.0) {
            eprintln!("ekko: V8 ICU set_common_data returned {code}; relying on any embedded data");
        }
...
fn css_module_op(
    ...
    match crate::parsers::css::extract_css_modules(&css, &path, None) {
        Ok(result) => {
...
pub fn setup_globals(scope: &mut v8::HandleScope) {
    ...
    bind_fn!(scope, web_ops, "readBodyChunk", crate::module_loader::web_read_body_chunk);
...
pub async fn execute_module_async(code: &str, filename: &str) -> anyhow::Result<()> {
    ...
    let module = {
        ...
        if ok.is_none() || ok == Some(false) {
            ...
            anyhow::bail!("{}", crate::module_loader::missing_export_hint(&msg));
        }
        module
    };
