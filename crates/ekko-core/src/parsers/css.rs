...
pub fn extract_css_modules(input: &str, filename: &str, pattern: Option<&str>) -> Result<CssModuleResult, String> {
    guard_nesting_depth(input)?;
    catch_parser(|| {
        use lightningcss::stylesheet::{ParserOptions, PrinterOptions, MinifyOptions};
        ...
        let pat = match pattern {
            Some(p) => Pattern::parse(p).map_err(|e| format!("css-modules: invalid pattern '{}': {:?}", p, e))?,
        ...
        if let Some(ref exports) = result.exports {
            ...
            for (name, export) in exports {
                let mut full = export.name.to_string();
                for cref in &export.composes {
                    match cref {
                        CssModuleReference::Local { name } => { full.push(' '); full.push_str(name); }
                        CssModuleReference::Global { name } => { full.push(' '); full.push_str(name); }
                        CssModuleReference::Dependency { .. } => {
                            return Err(format!(
                                "css-modules: cross-file `composes: … from \"…\"` is not supported (class '{}'); \
                                 compose within the same module or use a shared class", name));
                        }
                    }
                }
                classes.insert(name.to_string(), full);
            }
        }
...
pub const CSS_MODULE_PATTERN: &str = "[hash]_[local]";

pub fn convert_css_module(src: &str, logical_name: &str, syntax: SassSyntax) -> Result<CssModuleResult, String> {
    let css = compile_sass(src, syntax)?;
    extract_css_modules(&css, logical_name, Some(CSS_MODULE_PATTERN))
}
...
mod tests {
    ...
    fn fuzz_css_no_panic() {
        ...
        crate::fuzz_support::run_fuzz_cost("css", 20, |s| {
            ...
            let _ = extract_css_modules(s, "fuzz.css", None);
        });
    }
    ...
    fn css_modules_returns_code() {
        let result = extract_css_modules(".btn { color: red; }", "button.module.css", None).unwrap();
    ...
    #[test]
    fn convert_css_module_is_deterministic() {
        let scss = ".box { color: red; } .icon { width: 1rem; }";
        let a = convert_css_module(scss, "components/Button.module.scss", SassSyntax::Scss).unwrap();
        let b = convert_css_module(scss, "components/Button.module.scss", SassSyntax::Scss).unwrap();
        assert_eq!(a.classes, b.classes, "same logical name + source must give identical scoped names");
        assert_eq!(a.code, b.code, "scoped CSS must be byte-identical across calls");
        
        assert_ne!(a.classes["box"], "box");
        
        assert!(a.classes["box"].ends_with("_box"), "got {}", a.classes["box"]);
        assert!(a.classes["icon"].ends_with("_icon"), "got {}", a.classes["icon"]);
    }

    #[test]
    fn convert_css_module_two_files_distinct() {
        let a = convert_css_module(".box{color:red}", "a/Card.module.scss", SassSyntax::Scss).unwrap();
        let b = convert_css_module(".box{color:blue}", "b/Panel.module.scss", SassSyntax::Scss).unwrap();
        assert_ne!(a.classes["box"], b.classes["box"], "distinct files must scope `.box` to distinct names");
    }

    #[test]
    fn convert_css_module_logical_name_drives_scope() {
        let src = ".box{color:red}";
        let a = convert_css_module(src, "one/X.module.scss", SassSyntax::Scss).unwrap();
        let b = convert_css_module(src, "two/X.module.scss", SassSyntax::Scss).unwrap();
        assert_ne!(a.classes["box"], b.classes["box"]);
    }

    #[test]
    fn convert_css_module_composes_same_file() {
        let scss = ".base { padding: 4px; } .box { composes: base; color: red; }";
        let r = convert_css_module(scss, "c/Compose.module.scss", SassSyntax::Scss).unwrap();
        let box_val = &r.classes["box"];
        let base_val = &r.classes["base"];
        
        assert!(box_val.contains(base_val), "composes must append base scoped name: box='{}' base='{}'", box_val, base_val);
        assert!(box_val.split_whitespace().count() >= 2, "composed value should be space-joined: '{}'", box_val);
    }

    #[test]
    fn convert_css_module_compiles_scss_first() {
        let scss = "$p: #123456; .box { color: $p; &:hover { opacity: .5; } }";
        let r = convert_css_module(scss, "d/Nested.module.scss", SassSyntax::Scss).unwrap();
        assert!(r.code.contains("#123456"));
        assert!(r.classes.contains_key("box"));
    }

    #[test]
    fn convert_css_module_deep_nesting_errors_not_crash() {
        let deep = format!("{}color:red{}", ".a{".repeat(400), "}".repeat(400));
        assert!(convert_css_module(&deep, "e/Deep.module.scss", SassSyntax::Scss).is_err());
    }

    #[test]
    fn convert_css_module_cross_file_composes_errors() {
        let scss = ".box { composes: shared from \"./other.css\"; color: red; }";
        let r = convert_css_module(scss, "f/Cross.module.scss", SassSyntax::Scss);
        assert!(r.is_err(), "cross-file composes must error, got {:?}", r.map(|x| x.classes));
    }

    #[test]
    fn convert_css_module_proto_class_is_just_a_key() {
        let r = convert_css_module(".__proto__{color:red}.box{color:blue}", "g/Proto.module.scss", SassSyntax::Scss).unwrap();
        assert!(r.classes.contains_key("__proto__"));
        assert!(r.classes["__proto__"].ends_with("___proto__") || r.classes["__proto__"].contains("proto"));
    }
}
