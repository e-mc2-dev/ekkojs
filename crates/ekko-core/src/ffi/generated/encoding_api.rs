...
impl Api {
    ...
    pub fn encoding_base64Encode(&self, data: &[u8]) -> Result<String, String> {
        unsafe {
            let result = (self.ekko_encoding_base64Encode)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
    ...
    pub fn encoding_base64UrlEncode(&self, data: &[u8]) -> Result<String, String> {
        unsafe {
            let result = (self.ekko_encoding_base64UrlEncode)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn encoding_hexEncode(&self, data: &[u8]) -> Result<String, String> {
        unsafe {
            let result = (self.ekko_encoding_hexEncode)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn encoding_utf8Decode(&self, data: &[u8]) -> Result<String, String> {
        unsafe {
            let result = (self.ekko_encoding_utf8Decode)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn encoding_utf16LeDecode(&self, data: &[u8]) -> Result<String, String> {
        unsafe {
            let result = (self.ekko_encoding_utf16LeDecode)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn encoding_utf16BeDecode(&self, data: &[u8]) -> Result<String, String> {
        unsafe {
            let result = (self.ekko_encoding_utf16BeDecode)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
