...
impl Api {
    ...
    pub fn fs_write(&self, path: &str, data: &[u8]) -> Result<(), String> {
        unsafe {
            let result = (self.ekko_fs_write)(NativeString::from_str(path), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn fs_appendBytes(&self, path: &str, data: &[u8]) -> Result<(), String> {
        unsafe {
            let result = (self.ekko_fs_appendBytes)(NativeString::from_str(path), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn fs_handleWrite(&self, handle: i32, data: &[u8]) -> Result<i32, String> {
        unsafe {
            let result = (self.ekko_fs_handleWrite)(handle, NativeBuffer { ptr: data.as_ptr() as *mut u8, len: i32::try_from(data.len()).expect("buffer too large for i32") });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
