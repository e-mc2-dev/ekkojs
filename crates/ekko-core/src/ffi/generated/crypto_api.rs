...
impl Api {
    ...
    pub fn crypto_hash(&self, algorithm: &str, data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_hash)(NativeString::from_str(algorithm), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
    ...
    pub fn crypto_hmac(&self, algorithm: &str, key: &[u8], data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_hmac)(NativeString::from_str(algorithm), NativeBuffer { ptr: key.as_ptr() as *mut u8, len: key.len() as i32 }, NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_hashHex(&self, algorithm: &str, data: &[u8]) -> Result<String, String> {
        unsafe {
            let result = (self.ekko_crypto_hashHex)(NativeString::from_str(algorithm), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_encrypt(&self, algorithm: &str, key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_encrypt)(NativeString::from_str(algorithm), NativeBuffer { ptr: key.as_ptr() as *mut u8, len: key.len() as i32 }, NativeBuffer { ptr: plaintext.as_ptr() as *mut u8, len: plaintext.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_decrypt(&self, algorithm: &str, key: &[u8], data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_decrypt)(NativeString::from_str(algorithm), NativeBuffer { ptr: key.as_ptr() as *mut u8, len: key.len() as i32 }, NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_pbkdf2(&self, password: &[u8], salt: &[u8], iterations: i32, hash: &str, length: i32) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_pbkdf2)(NativeBuffer { ptr: password.as_ptr() as *mut u8, len: password.len() as i32 }, NativeBuffer { ptr: salt.as_ptr() as *mut u8, len: salt.len() as i32 }, iterations, NativeString::from_str(hash), length);
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_hkdf(&self, ikm: &[u8], salt: &[u8], info: &[u8], hash: &str, length: i32) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_hkdf)(NativeBuffer { ptr: ikm.as_ptr() as *mut u8, len: ikm.len() as i32 }, NativeBuffer { ptr: salt.as_ptr() as *mut u8, len: salt.len() as i32 }, NativeBuffer { ptr: info.as_ptr() as *mut u8, len: info.len() as i32 }, NativeString::from_str(hash), length);
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_rsaSign(&self, privatePem: &str, data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_rsaSign)(NativeString::from_str(privatePem), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_rsaVerify(&self, publicPem: &str, data: &[u8], signature: &[u8]) -> Result<bool, String> {
        unsafe {
            let result = (self.ekko_crypto_rsaVerify)(NativeString::from_str(publicPem), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 }, NativeBuffer { ptr: signature.as_ptr() as *mut u8, len: signature.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_ecdsaSign(&self, privatePem: &str, data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_crypto_ecdsaSign)(NativeString::from_str(privatePem), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn crypto_ecdsaVerify(&self, publicPem: &str, data: &[u8], signature: &[u8]) -> Result<bool, String> {
        unsafe {
            let result = (self.ekko_crypto_ecdsaVerify)(NativeString::from_str(publicPem), NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 }, NativeBuffer { ptr: signature.as_ptr() as *mut u8, len: signature.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
