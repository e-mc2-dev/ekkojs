...
impl Api {
    ...
    pub fn webserver_sendResponse(&self, requestId: i32, status: i32, headersJson: &str, body: &[u8]) -> Result<(), String> {
        unsafe {
            let result = (self.ekko_webserver_sendResponse)(requestId, status, NativeString::from_str(headersJson), NativeBuffer { ptr: body.as_ptr() as *mut u8, len: body.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
