...
impl Api {
    ...
    pub fn compress_gzipCompress(&self, data: &[u8], level: i32) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_compress_gzipCompress)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 }, level);
            if result.is_ok == 0 {
    ...
    pub fn compress_gzipDecompress(&self, data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_compress_gzipDecompress)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn compress_brotliCompress(&self, data: &[u8], level: i32) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_compress_brotliCompress)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 }, level);
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn compress_brotliDecompress(&self, data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_compress_brotliDecompress)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn compress_deflateCompress(&self, data: &[u8], level: i32) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_compress_deflateCompress)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 }, level);
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn compress_deflateDecompress(&self, data: &[u8]) -> Result<Vec<u8>, String> {
        unsafe {
            let result = (self.ekko_compress_deflateDecompress)(NativeBuffer { ptr: data.as_ptr() as *mut u8, len: data.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
    ...
    pub fn compress_streamWrite(&self, handle: i32, chunk: &[u8]) -> Result<(), String> {
        unsafe {
            let result = (self.ekko_compress_streamWrite)(handle, NativeBuffer { ptr: chunk.as_ptr() as *mut u8, len: chunk.len() as i32 });
            if result.is_ok == 0 {
                let msg = result.error_message.to_string_lossy();
                (self.free_string)(result.error_message);
                return Err(msg);
