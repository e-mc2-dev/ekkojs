...
function createServer(opts) {
  ...
  function handleRoute(req, res, handle) {
    ...
    for (var si = 0; si < statics.length; si++) {
      var sd = statics[si];
      if (req.path.startsWith(sd.prefix + '/') || req.path === sd.prefix) {
        var rel = req.path.slice(sd.prefix.length).replace(/^\//, '');
    ...
    if (!matched) { res.status(404).text('Not Found'); return; }
      ...
      function runRouteM() {
        if (idx < mOpts.middleware.length) {
          ...
          try {
            ...
            if (r && r.then) r.catch(function(e) { if (!res._sent) { if (_onError) try { _onError(e, req, res); } catch (e2) {} if (!res._sent) res.status(500).json({ error: String(e) }); } });
          } catch (e) { if (_onError) try { _onError(e, req, res); } catch (e2) {} if (!res._sent) res.status(500).json({ error: String(e) }); }
    ...
    try {
      ...
      if (result && result.then) {
        result.then(function() { if (!res._sent) ops.sendResponse(handle, res._s, JSON.stringify(res._h), ''); })
              .catch(function(e) { if (_onError) try { _onError(e, req, res); } catch (e2) {} if (!res._sent) res.status(500).json({ error: String(e) }); });
  ...
  function dispatch(json, handle) {
    ...
    if (req.bodyEncoding === 'base64' && req.body) { req.bodyBytes = b64dec(req.body); }
    else if (req.headers && req.headers['content-type'] === 'text/base64' && req.body) { req.bodyBytes = b64dec(req.body); }
    req.cookies = {};
    ...
    req.json = function() { try { return JSON.parse(this.body || 'null'); } catch (e) { return null; } };
    req.text = function() { return this.body || ''; };
    req.bytes = function() { return this.bodyBytes || new TextEncoder().encode(this.body || ''); };
...
function bodyLimit(opts) {
  ...
  return function(req, res, next) {
    ...
    var actual = req.bodyBytes ? req.bodyBytes.length : (req.body ? req.body.length : 0);
    if (cl > max || actual > max) { res.status(413).json({ error: 'Payload Too Large', maxBytes: max }); return; }
    next();
  };
}
...
function errorHandler(opts) {
  ...
  return function(req, res, next) {
    try {
      var r = next();
      if (r && r.then) r.catch(function(e) { if (!res._sent) { console.error('[ERROR]', req.method, req.path, String(e)); res.status(500).json({ error: prod ? 'Internal Server Error' : String(e) }); } });
    } catch (e) { if (!res._sent) { console.error('[ERROR]', req.method, req.path, String(e)); res.status(500).json({ error: prod ? 'Internal Server Error' : String(e) }); } }
  };
}
