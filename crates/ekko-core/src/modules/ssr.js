...
function serializeProps(value){
  ...
  function _ser(val,path){
    ...
    if(t==='object'){
      ...
      seen.add(val);
      if(Array.isArray(val)){var _arr=val.map(function(item,i){return _ser(item,path+'['+i+']');});seen.delete(val);return _arr;}
      var result={};
...
function htmlShell(opts){
  ...
  const xo=s=>/^https?:\/\//i.test(String(s))?' crossorigin':'';
  return '<!DOCTYPE html><html lang="'+escapeHtml(String(lang))+'"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">'
    ...
    +styles.map(s=>'<link rel="stylesheet" href="'+escapeHtml(String(s))+'"'+xo(s)+'>').join('')
    +inlineStyles
    +preloads.map(s=>'<link rel="modulepreload" href="'+escapeHtml(String(s))+'"'+xo(s)+'>').join('')
    +head
    ...
    +modules.map(s=>'<script type="module" src="'+escapeHtml(String(s))+'"'+xo(s)+'></script>').join('')
    +'</body></html>';
}
