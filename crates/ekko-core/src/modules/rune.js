...
function resolvePageAssets(manifest,pageKey,prefix){
  prefix=prefix||'/_ekko/';
  var result={modules:[],modulepreload:[],pageFile:null,styles:[]};
  if(manifest.hydrate){result.modules.push(prefix+manifest.hydrate);result.modulepreload.push(prefix+manifest.hydrate);var hEntry=manifest.pages&&manifest.pages['_hydrate.js'];if(hEntry&&hEntry.imports){for(var i=0;i<hEntry.imports.length;i++)result.modulepreload.push(prefix+hEntry.imports[i]);}}
  var globalStyles=manifest.styles||[];
  for(var i=0;i<globalStyles.length;i++)result.styles.push(prefix+globalStyles[i]);
  ...
  if(entry){
    ...
    result.pageFile=prefix+file;
    result.modulepreload.push(prefix+file);
    var imports=entry.imports||[];
    for(var i=0;i<imports.length;i++)result.modulepreload.push(prefix+imports[i]);
    var css=entry.css||[];
    for(var i=0;i<css.length;i++)result.styles.push(prefix+css[i]);
  }
  ...
  if(layoutKey){var le=manifest.pages[layoutKey];if(le){result.modulepreload.push(prefix+(typeof le==='string'?le:le.file));if(le.imports){for(var i=0;i<le.imports.length;i++)result.modulepreload.push(prefix+le.imports[i]);}}}
...
function createApp(opts){
  ...
  function _resolveAssets(route){
    ...
    var pageFile=null;
    if(manifest&&pageKey){
      var assets=resolvePageAssets(manifest,pageKey);
  ...
  const app={
    ...
    start(){
      ...
      try{var __hmrFile='.ekko/build/client/__hmr';var __hmrId=null;try{__hmrId=ops.readText(__hmrFile).trim();}catch(e){}setInterval(function(){try{var id=ops.readText(__hmrFile).trim();if(__hmrId&&id!==__hmrId){__hmrId=id;for(var i=0;i<__hmrSockets.length;i++){try{__hmrSockets[i].send('reload');}catch(e){}}}}catch(e){}},300);}catch(e){}
      ...
      for(const api of apiRoutes){
        if(api.method==='__405'){
          var _allowed=api.allowed;
          ['GET','POST','PUT','DELETE','PATCH'].forEach(function(m){
            if(_allowed.indexOf(m)<0){
              var register=m==='GET'?server.get:m==='POST'?server.post:m==='PUT'?server.put:m==='DELETE'?server.delete:null;
              if(register)register.call(server,api.path,function(req,res){res.status(405).json({error:'Method Not Allowed',allowed:_allowed});});
            }
          });
        }
        else if(api.method==='GET')server.get(api.path,api.opts||{},wrapApiHandler(api.handler));
      ...
      if(manifest&&manifest.pages){for(var _lk in manifest.pages){if(_lk.match(/layout\.(tsx|jsx|ts|js)$/)&&!_lk.match(/\//)){var _le=manifest.pages[_lk];_layoutFile='/_ekko/'+(typeof _le==='string'?_le:_le.file);break;}}}
      _clientRoutes=[];
      for(var _ri=0;_ri<routes.length;_ri++){
        var _r=routes[_ri];var _pk=_r.meta.page||null;var _pf=null;
        if(manifest&&_pk){var _a=resolvePageAssets(manifest,_pk);_pf=_a.pageFile;}
      ...
      return{server:server,url:url,stop:function(){server.stop();},invalidate:app.invalidate,cache:_cache};
    },
