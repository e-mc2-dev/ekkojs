...
var __DEV=(function(){try{return Ekko.env.get("EKKO_DEV")==="1";}catch(e){return false;}})();
var __DEV_TOKEN=__DEV?(function(){try{return Ekko.env.get("EKKO_DEV_TOKEN")||"";}catch(e){return "";}})():"";
var __DEV_RELOAD_TAG=__DEV?'<script>window.__ekkoDev=1</script>':'';
...
function resolvePageAssets(manifest,pageKey,prefix,cdn){
  ...
  if(!entry&&manifest.pages&&pageKey){
    ...
    if(!entry){var _bn=_stem.split('/').pop();var _hits=[];for(var _pk2 in manifest.pages){if(_pk2.replace(/\.(tsx|jsx|ts|js|mjs)$/,'').split('/').pop()===_bn)_hits.push(_pk2);}if(_hits.length===1)entry=manifest.pages[_hits[0]];}
  }
  if(entry){
...
function createApp(opts){
  ...
  async function _ssrRender(route){
    ...
    var ssrResult={};
    if(ssrFn){ssrResult=(await ssrFn())||{};}
    ...
    var html=htmlShell({lang:_lang,title:ssrResult.title||route.meta.title||'EkkoJS',body:body,styles:a.pageStyles,scripts:opts.scripts,modules:a.pageModules,data:data,head:(ssrResult.head||route.meta.head||'')+__DEV_RELOAD_TAG,modulepreload:a.pagePreloads,inlineStyles:route.meta.inlineStyles||''});
  ...
  function _serveShell(route,req,res){
    ...
    var html=htmlShell({lang:_lang,title:route.meta.title||'EkkoJS',body:body,styles:a.pageStyles,scripts:opts.scripts,modules:a.pageModules,data:data,head:(route.meta.head||'')+__DEV_RELOAD_TAG,modulepreload:a.pagePreloads});
    res.html(html);
  }
  ...
  const app={
    ...
    async invalidate(pathOrTag){
      if(pathOrTag==='*'){
        _cache.clear();
        for(var i=0;i<routes.length;i++){var r=routes[i];var hasSsr=r.component.ssr||(r.component.default&&r.component.default.ssr)||null;var isDyn=r.path.indexOf(':')>=0||r.path.indexOf('*')>=0;if(hasSsr&&!isDyn)await _ssrRender(r);}
        return;
      }
      if(pathOrTag.charAt(0)==='/'){
        _cache.delete(pathOrTag);
        for(var i=0;i<routes.length;i++){var r=routes[i];if(r.path===pathOrTag){var hasSsr=r.component.ssr||(r.component.default&&r.component.default.ssr)||null;if(hasSsr)await _ssrRender(r);}}
        return;
      }
      for(var entry of _cache){if(entry[1].tags&&entry[1].tags.indexOf(pathOrTag)>=0)_cache.delete(entry[0]);}
      for(var i=0;i<routes.length;i++){var r=routes[i];if(r.meta.tags&&r.meta.tags.indexOf(pathOrTag)>=0){var hasSsr=r.component.ssr||(r.component.default&&r.component.default.ssr)||null;var isDyn=r.path.indexOf(':')>=0||r.path.indexOf('*')>=0;if(hasSsr&&!isDyn)await _ssrRender(r);}}
    },
    start(){
      ...
      server.static('/_ekko','.ekko/build/client',{maxAge:31536000,immutable:true,before:true});
      var __hmrSockets=[];
      var __hmrTimer=null;
      ...
      if(__DEV){
        ...
        server.post('/__ekko_dev/shutdown',{},function(req,res){
          var tok=(req.headers&&(req.headers['x-ekko-dev-token']||req.headers['X-Ekko-Dev-Token']))||'';
          if(!__DEV_TOKEN||tok!==__DEV_TOKEN){res.status(403).text('forbidden');return;}
          res.status(200).text('ok');
          
          setTimeout(function(){try{if(__hmrTimer)clearInterval(__hmrTimer);}catch(e){}try{server.stop();}catch(e){}try{Ekko.exit(0);}catch(e){}},40);
        });
      }
      function wrapApiHandler(handler){
        ...
        function finishApi(result,res){
          if(result!==undefined&&!res._sent){
            if(typeof result==='object'&&result!==null)res.json(result);
            else res.send(String(result));
          }
        }
        function failApi(e,res){console.error('[API error]',String(e.stack||e));if(!res._sent)res.status(500).json({error:String(e.message||e)});}
        return function(req,res){
          try{
            var result=handler(req,res);
            if(result&&typeof result.then==='function'){
              return result.then(function(r){finishApi(r,res);}).catch(function(e){failApi(e,res);});
            }
            finishApi(result,res);
          }catch(e){failApi(e,res);}
        };
      }
      ...
      for(var _ri=0;_ri<routes.length;_ri++){
        ...
        if(manifest&&_pk){var _a=resolvePageAssets(manifest,_pk,'/_ekko/',_cdn);_pf=_a.pageFile;
          ...
          if(!_pf){console.warn('[rune] route "'+_r.path+'" (page "'+_pk+'") has NO client chunk in the build manifest — it will render blank or force a full reload. Try a clean build: remove .ekko/build then `ekko build --client`. Manifest pages: ['+Object.keys((manifest&&manifest.pages)||{}).join(', ')+']');}
        }
      ...
      for(const route of routes){
        ...
        if(_hasSsr&&!_isDynamic){
          (function(route){server.get(route.path,async function(req,res){
            ...
            try{res.html(await _ssrRender(route));}catch(e){console.error('[SSR error]',route.path,String(e.stack||e));
      ...
      if(_ssrStrategy==='eager'||ssrRoutes.length>0){
        ...
        if(eagerRoutes.length>0){
          Promise.all(eagerRoutes.map(function(r){return Promise.resolve().then(function(){return _ssrRender(r);}).catch(function(e){console.error('SSR error for '+r.path+': '+e.message);});}))
            .then(function(){console.log('SSR cache: '+eagerRoutes.length+' pages rendered (eager)');});
        }
      }
      var bgRoutes=ssrRoutes.filter(function(r){return(r.meta.ssr||_ssrStrategy)==='background';});
      if(bgRoutes.length>0){
        setTimeout(async function(){
          for(var i=0;i<bgRoutes.length;i++){if(!_cache.has(bgRoutes[i].path)){try{await _ssrRender(bgRoutes[i]);}catch(e){console.error('SSR bg error for '+bgRoutes[i].path+': '+e.message);}}}
...
return{createApp:createApp,scanRoutes:scanRoutes,readManifest:readManifest,resolvePageAssets:resolvePageAssets,composeLayouts:composeLayouts,
