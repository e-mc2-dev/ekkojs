...
function _isInsertSql(sql){return /^\s*insert\b/i.test(sql);}
function _sqliteLastId(db){try{var r=db.query('SELECT last_insert_rowid() AS id');return r&&r.rows&&r.rows[0]?r.rows[0][0]:0;}catch(e){return 0;}}
...
SqliteClient.prototype.exec=function(sql,params){
  ...
  var n;
  if(p){var stmt=this._db.prepare(sql);n=stmt.exec(p)||0;stmt.close();}
  else{n=this._db.exec(sql)||0;}
  var res={affectedRows:n};
  if(_isInsertSql(sql))res.lastInsertId=_sqliteLastId(this._db);
  return res;
};
...
function connect(nameOrDb,config){
  ...
  var makeConn=function(){
    if(typeof nameOrDb==='string'){
      ...
      if(/[\/\\.:]/.test(nameOrDb))return new SqliteConnection(nameOrDb);
      throw new Error("Unknown ORM driver '"+nameOrDb+"'. Register it with orm.registerDriver('"+nameOrDb+"', factory) — driver packages (postgres/mysql/mssql/mongodb) call register() for you — or pass a SQLite target such as ':memory:' or a file path like './app.db'.");
    }
