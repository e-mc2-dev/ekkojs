...
function parseOptionFlags(flags){
  ...
  for(var i=0;i<parts.length;i++){
    ...
    var m=parts[i].match(/^(--?[A-Za-z0-9][A-Za-z0-9-]*)(?:[ =]+[<\[]?([A-Za-z0-9_-]+)[>\]]?)?$/);
    if(!m)continue;
    var flag=m[1];
    if(m[2])argName=m[2];
    if(flag.indexOf('--')===0)long=flag.slice(2);
    else short=flag.slice(1);
  }
...
function cli(name,version){
  ...
  var app={_name:name,_version:version,name:name,version:version,_description:'',_examples:[],_commands:[]};
...
function blinkCursor(){showCursor();_w(E+'5 q');}
function resetCursor(){_w(E+'0 q');}
async function promptInput(message,opts){
  ...
  try{
    while(true){
      ...
      if(k.key==='Enter'){resetCursor();_w('\n');return value;}
      if(k.key==='Esc'||(k.key==='Char'&&k.char==='c'&&k.ctrl)){resetCursor();_w('\n');return null;}
      if(k.key==='Backspace'){
  ...
  }catch(e){resetCursor();showCursor();throw e;}
}
...
async function promptSelect(message,choices,opts){
  ...
  function render(){
    ...
    for(var i=0;i<vis.length;i++){
      ...
      _w((sel?cyan('❯ '+label):dim('  ')+label)+E+'K\n');
    }
...
var _fgCode={black:30,red:31,green:32,yellow:33,blue:34,magenta:35,cyan:36,white:37,gray:90,brightBlack:90,brightRed:91,brightGreen:92,brightYellow:93,brightBlue:94,brightMagenta:95,brightCyan:96,brightWhite:97};
var _bgCode={black:40,red:41,green:42,yellow:43,blue:44,magenta:45,cyan:46,white:47,gray:100,brightBlack:100,brightRed:101,brightGreen:102,brightYellow:103,brightBlue:104,brightMagenta:105,brightCyan:106,brightWhite:107};
function _rep(c,n){var s='';for(var i=0;i<n;i++)s+=c;return s;}
function _colorSeq(name,isBg){
  if(!name)return null;
  if(name.charAt(0)==='#'&&name.length===7){var r=parseInt(name.slice(1,3),16),g=parseInt(name.slice(3,5),16),b=parseInt(name.slice(5,7),16);return (isBg?'48':'38')+';2;'+r+';'+g+';'+b;}
  var m=isBg?_bgCode:_fgCode;return m[name]!=null?String(m[name]):null;
}
function _sgr(text,o){
  o=o||{};var codes=[];
  if(o.bold)codes.push('1');if(o.italic)codes.push('3');if(o.underline)codes.push('4');
  var fg=_colorSeq(o.fg,false);if(fg)codes.push(fg);
  var bg=_colorSeq(o.bg,true);if(bg)codes.push(bg);
  if(!codes.length)return text;
  return E+codes.join(';')+'m'+text+E+'0m';
}
function _align(s,w,a){
  s=String(s);if(s.length>=w)return s;
  var d=w-s.length;
  if(a==='right')return _rep(' ',d)+s;
  if(a==='middle'||a==='center'){var l=Math.floor(d/2);return _rep(' ',l)+s+_rep(' ',d-l);}
  return s+_rep(' ',d);
}

var _mdTheme={Plain:null,Keyword:'magenta',Type:'cyan',String:'green',Number:'yellow',Comment:'gray',Operator:null,Punctuation:null,Identifier:null,Function:'blue',Preprocessor:'yellow',Heading:'cyan',Bold:'white',Italic:'cyan',BoldItalic:'cyan',Link:'blue',CodeSpan:'green'};
function _highlightCode(code,lang){
  var lines=code.split('\n');var tokensByLine=null;
  if(ops.tokenize){try{tokensByLine=JSON.parse(ops.tokenize(lang||'',code));}catch(e){tokensByLine=null;}}
  if(!tokensByLine||!tokensByLine.length)return lines.map(function(l){return dim(l);});
  var out=[];
  for(var i=0;i<lines.length;i++){
    var line=lines[i];var toks=tokensByLine[i]||[];var pos=0;var buf='';
    for(var t=0;t<toks.length;t++){
      var tok=toks[t];var start=tok.start||0;var len=tok.length||0;var kind=tok.kind||'Plain';
      if(start>pos)buf+=line.slice(pos,start);
      var color=_mdTheme[kind];var seg=line.slice(start,start+len);
      buf+=color?_sgr(seg,{fg:color}):seg;pos=start+len;
    }
    if(pos<line.length)buf+=line.slice(pos);
    out.push(buf);
  }
  return out;
}
function _mdInline(s){
  s=s.replace(/`([^`]+)`/g,function(_m,c){return green(c);});
  s=s.replace(/\[([^\]]+)\]\(([^)]+)\)/g,function(_m,t){return underline(blue(t));});
  s=s.replace(/\*\*([^*]+)\*\*/g,function(_m,b){return bold(b);});
  s=s.replace(/__([^_]+)__/g,function(_m,b){return bold(b);});
  s=s.replace(/\*([^*]+)\*/g,function(_m,it){return italic(it);});
  return s;
}
function renderMarkdownAnsi(content){
  var lines=String(content).replace(/\r\n/g,'\n').split('\n');var out=[];var i=0;
  while(i<lines.length){
    var line=lines[i];var fence=line.match(/^```(\w*)\s*$/);
    if(fence){
      var lang=fence[1];var code=[];i++;
      while(i<lines.length&&!/^```\s*$/.test(lines[i])){code.push(lines[i]);i++;}
      i++;
      var hl=_highlightCode(code.join('\n'),lang);
      out.push(dim('─── '+(lang||'code')+' ───'));
      for(var c=0;c<hl.length;c++)out.push('  '+hl[c]);
      out.push(dim('───'));continue;
    }
    var h=line.match(/^(#{1,6})\s+(.*)$/);
    if(h){var lvl=h[1].length;var txt=_mdInline(h[2]);out.push(lvl<=2?bold(cyan(txt)):bold(txt));i++;continue;}
    if(/^\s*([-*_])\1{2,}\s*$/.test(line)){out.push(dim(_rep('─',Math.min(columns(),40))));i++;continue;}
    var bq=line.match(/^>\s?(.*)$/);
    if(bq){out.push(dim('│ ')+_mdInline(bq[1]));i++;continue;}
    var li=line.match(/^(\s*)[-*+]\s+(.*)$/);
    if(li){out.push(li[1]+cyan('• ')+_mdInline(li[2]));i++;continue;}
    var oli=line.match(/^(\s*)(\d+)\.\s+(.*)$/);
    if(oli){out.push(oli[1]+cyan(oli[2]+'. ')+_mdInline(oli[3]));i++;continue;}
    out.push(_mdInline(line));i++;
  }
  return out.join('\n');
}
...
function renderCsvAnsi(opts){
  opts=opts||{};var sep=opts.separator||',';var noHeader=!!opts.noHeader;
  var text=String(opts.csv||'').replace(/\r\n/g,'\n').replace(/\n+$/,'');
  if(!text)return '';
  var rows=text.split('\n').map(function(l){return _parseCsvLine(l,sep);});
  var n=0;rows.forEach(function(r){if(r.length>n)n=r.length;});
  var widths=[];for(var c=0;c<n;c++){var w=0;rows.forEach(function(r){var v=String(r[c]==null?'':r[c]);if(v.length>w)w=v.length;});widths.push(w);}
  function rowStr(r,head){var cells=[];for(var c=0;c<n;c++){var v=_align(r[c]==null?'':r[c],widths[c],'left');cells.push(head?bold(v):v);}return '│ '+cells.join(' │ ')+' │';}
  var out=[];var start=0;
  if(!noHeader&&rows.length){out.push(rowStr(rows[0],true));out.push('├─'+widths.map(function(w){return _rep('─',w);}).join('─┼─')+'─┤');start=1;}
  for(var r=start;r<rows.length;r++)out.push(rowStr(rows[r],false));
  return out.join('\n');
}

function renderTableAnsi(opts){
  opts=opts||{};var cols=opts.cols||[];var data=opts.data||[];var n=cols.length;
  var widths=[];for(var c=0;c<n;c++){var w=String(cols[c].text==null?'':cols[c].text).length;for(var r=0;r<data.length;r++){var v=String(data[r]&&data[r][c]!=null?data[r][c]:'');if(v.length>w)w=v.length;}widths.push(w);}
  var out=[];var hc=[];
  for(var c=0;c<n;c++){var col=cols[c];var txt=_align(col.text==null?'':col.text,widths[c],col.align||'left');hc.push(_sgr(txt,{fg:col.fgColor,bg:col.bgColor,bold:col.bold,italic:col.italic}));}
  out.push('│ '+hc.join(' │ ')+' │');
  out.push('├─'+widths.map(function(w){return _rep('─',w);}).join('─┼─')+'─┤');
  for(var r=0;r<data.length;r++){var dc=[];for(var c=0;c<n;c++){var col=cols[c];dc.push(_align(data[r]&&data[r][c]!=null?data[r][c]:'',widths[c],col.align||'left'));}out.push('│ '+dc.join(' │ ')+' │');}
  return out.join('\n');
}

var render={
  markdown:function(content){_w(renderMarkdownAnsi(content)+'\n');},
  csv:function(opts){_w(renderCsvAnsi(opts)+'\n');},
  table:function(opts){_w(renderTableAnsi(opts)+'\n');},
};

return{cli:cli,prompt:prompt,render:render};
})
