...
struct RootConfig {
    ...
    #[serde(rename = "maxEkko")]
    max_ekko: Option<String>,
    private: Option<bool>,           
    publish: Option<String>,         
    storage: Option<String>,         
    peer: Option<HashMap<String, String>>,  
    x: Option<HashMap<String, String>>,     
}
...
const KNOWN_FIELDS: &[&str] = &[
    ...
    "description", "license", "homepage", "repository", "readme", "minEkko", "maxEkko", "private",
...
pub fn lookup_run_alias(start_dir: &Path, name: &str) -> Option<(String, PathBuf)> {
    let mut dir = Some(start_dir);
    while let Some(d) = dir {
        let cfg = d.join("ekko.json");
        if cfg.exists() {
            if let Some(config) = read_ekko_json(&cfg) {
                if let Some(v) = config.x.as_ref().and_then(|m| m.get(name)) {
                    return Some((v.clone(), d.to_path_buf()));
                }
            }
        }
        dir = d.parent();
...
pub fn run_aliases_from_dir(start_dir: &Path) -> HashMap<String, String> {
    ...
    while let Some(d) = dir {
        ...
        if cfg.exists() {
            if let Some(config) = read_ekko_json(&cfg) {
                if let Some(m) = config.x {
                    for (k, v) in m { acc.entry(k).or_insert(v); } 
                }
            }
        }
        dir = d.parent();
...
fn parse_member(global_name: &str, member_dir: &Path, config: RootConfig) -> Option<Member> {
    let kind = match config.kind.as_deref() {
        ...
        Some("rune") => MemberKind::Rune,
        Some("web") => MemberKind::Web,
...
mod tests {
    ...
    #[test]
    fn parse_member_kind_dispatch_m35() {
        
        assert_eq!(parse(r#"{ "name": "r", "version": "1.0.0", "type": "rune", "entry": "server.tsx" }"#).kind, MemberKind::Rune);
        assert_eq!(parse(r#"{ "name": "w", "version": "1.0.0", "type": "web", "entry": "serve.ts" }"#).kind, MemberKind::Web);
        assert_eq!(parse(r#"{ "name": "x", "version": "1.0.0", "type": "run", "entry": "main.ts" }"#).kind, MemberKind::Run);
        
        let cfg: RootConfig = serde_json::from_str(r#"{ "name": "u", "version": "1.0.0", "type": "frobnicate" }"#).unwrap();
        assert!(parse_member("u", Path::new("/tmp/u"), cfg).is_none(), "unknown type must return None");
    }
...
"version": "1.0.0" }"#;
    ...
    fn mk_member(name: &str, root: &str) -> Member {
        Member {
            ...
            description: None, license: None, repository: None, readme: None, min_ekko: None, max_ekko: None,
    ...
    #[test]
    fn root_config_parses_x_aliases() {
        
        let cfg: RootConfig = serde_json::from_str(
            r#"{ "name":"p","version":"1.0.0","type":"run","entry":"m.ts",
                 "x": { "smtp":"@ekkojs/smtp-dev", "dev":"./scripts/dev.ts" } }"#).unwrap();
        let x = cfg.x.unwrap();
        assert_eq!(x.get("smtp").unwrap(), "@ekkojs/smtp-dev");
        assert_eq!(x.get("dev").unwrap(), "./scripts/dev.ts");
    }

    #[test]
    fn known_fields_contains_x() {
        
        assert!(KNOWN_FIELDS.contains(&"x"));
    }

    #[test]
    fn lookup_run_alias_walks_up_and_resolves() {
        let tmp = std::env::temp_dir().join("ekko_test_x_walkup");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(tmp.join("apps/api/src")).unwrap();
        let tmp = fs::canonicalize(&tmp).unwrap();
        
        fs::write(tmp.join("ekko.json"),
            r#"{ "name":"root","version":"1.0.0","type":"run","entry":"m.ts",
                 "x": { "smtp":"@ekkojs/smtp-dev" } }"#).unwrap();

        assert_eq!(lookup_run_alias(&tmp, "smtp").map(|(v, _)| v).as_deref(), Some("@ekkojs/smtp-dev"));
        assert_eq!(lookup_run_alias(&tmp.join("apps/api/src"), "smtp").map(|(v, _)| v).as_deref(), Some("@ekkojs/smtp-dev"));
        
        assert_eq!(lookup_run_alias(&tmp.join("apps/api/src"), "smtp").map(|(_, d)| d), Some(tmp.clone()));
        
        assert!(lookup_run_alias(&tmp.join("apps/api/src"), "nope").is_none());

        let _ = fs::remove_dir_all(&tmp);
    }

    #[test]
    fn lookup_run_alias_nearer_wins() {
        let tmp = std::env::temp_dir().join("ekko_test_x_nearer");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(tmp.join("member")).unwrap();
        let tmp = fs::canonicalize(&tmp).unwrap();
        fs::write(tmp.join("ekko.json"),
            r#"{ "name":"root","version":"1.0.0","type":"run","entry":"m.ts",
                 "x": { "dev":"./root-dev.ts", "seed":"@org/seeder" } }"#).unwrap();
        
        fs::write(tmp.join("member/ekko.json"),
            r#"{ "name":"m","version":"1.0.0","type":"run","entry":"m.ts",
                 "x": { "dev":"./member-dev.ts" } }"#).unwrap();

        assert_eq!(lookup_run_alias(&tmp.join("member"), "dev").map(|(v, _)| v).as_deref(), Some("./member-dev.ts"));
        assert_eq!(lookup_run_alias(&tmp.join("member"), "seed").map(|(v, _)| v).as_deref(), Some("@org/seeder"));
        
        assert_eq!(lookup_run_alias(&tmp.join("member"), "dev").map(|(_, d)| d), Some(tmp.join("member")));
        assert_eq!(lookup_run_alias(&tmp.join("member"), "seed").map(|(_, d)| d), Some(tmp.clone()));

        let merged = run_aliases_from_dir(&tmp.join("member"));
        assert_eq!(merged.get("dev").unwrap(), "./member-dev.ts"); 
        assert_eq!(merged.get("seed").unwrap(), "@org/seeder");     

        let _ = fs::remove_dir_all(&tmp);
    }

    #[test]
    fn lookup_run_alias_no_ekko_json_returns_none() {
        let tmp = std::env::temp_dir().join("ekko_test_x_none");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
        let tmp = fs::canonicalize(&tmp).unwrap();
        
        assert!(lookup_run_alias(&tmp, "smtp-xyz-unlikely").is_none());
        assert!(run_aliases_from_dir(&tmp).get("smtp-xyz-unlikely").is_none());
        let _ = fs::remove_dir_all(&tmp);
    }
}
