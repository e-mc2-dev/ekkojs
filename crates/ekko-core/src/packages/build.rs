...
pub fn pack_member_for(member: &Member, platform: &str, arch: &str, platform_key: &str) -> anyhow::Result<Vec<u8>> {
    ...
    let project_type = match member.kind {
        MemberKind::Run => "run", MemberKind::Lib => "lib", MemberKind::Tool => "tool",
...
mod tests {
    ...
    fn pack_includes_readme_any_name() {
        ...
        let mut member = Member {
            ...
            description: None, license: None, repository: None, readme: Some("DOC.md".into()), min_ekko: None,
            publish: "public".into(), storage: "public".into(), peer: HashMap::new(),
        };
