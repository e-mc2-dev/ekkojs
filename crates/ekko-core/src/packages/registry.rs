...
impl RegistryClient {
    ...
    pub fn download(&self, name: &str, version: &str, platform: &str, arch: &str, dest: &Path) -> anyhow::Result<PathBuf> {
        ...
        let data = match fetch(platform, arch) {
            Ok(d) => d,
            Err(e) => {
                if platform == "any" && arch == "any" { return Err(e); }
                fetch("any", "any").map_err(|_| e)?
            }
        };
    ...
    pub fn publish(&self, name: &str, version: &str, platform: &str, arch: &str, ekl_path: &Path, signature: Option<&str>, min_ekko: Option<&str>, visibility: &str, storage: &str, allow_private_on_public: bool) -> anyhow::Result<PublishResponse> {
        ...
        let body = http_put_binary(&url, token, &data, signature, min_ekko, visibility, storage, allow_private_on_public)?;
        Ok(serde_json::from_str(&body)?)
    }
    ...
    pub fn publish_meta(&self, name: &str, version: &str, readme: &str, description: &str, repository: &str, license: &str) -> anyhow::Result<()> {
        let token = self.config.token.as_ref()
            .ok_or_else(|| anyhow::anyhow!("not authenticated. Run: ekko login"))?;
        let url = format!(
            "{}/v1/packages/{}/{}/meta",
...
fn http_put_binary(url: &str, token: &str, data: &[u8], signature: Option<&str>, min_ekko: Option<&str>, visibility: &str, storage: &str, allow_private_on_public: bool) -> anyhow::Result<String> {
    let api = load_web_api()?;
