...
pub fn fetch_extract(
    ...
    if let Some(min_ekko) = summary.versions.iter()
        .find(|v| v.version == resolved_version)
        .map(|v| v.min_ekko_version.clone())
        .filter(|m| !m.is_empty())
    {
        if !version_gte(crate::VERSION, &min_ekko) {
            anyhow::bail!(
                "package '{}@{}' requires EkkoJS >= {} (you have {}). Upgrade the runtime to install it.",
                name, resolved_version, min_ekko, crate::VERSION
            );
        }
    }
...
pub fn publish_package(
    ...
    for (platform, arch, key) in &platforms {
        ...
        match client.publish(&member.name, &member.version, platform, arch, &ekl_path, signature.as_deref(), member.min_ekko.as_deref(), &member.publish, &member.storage, allow_private_on_public) {
            Ok(_) => {}
            Err(e) => {
                let _ = std::fs::remove_file(&ekl_path);
    ...
    if !readme_content.is_empty() || !description.is_empty() || !repository.is_empty() {
        if let Err(e) = client.publish_meta(&member.name, &member.version, &readme_content, &description, &repository, &license) {
