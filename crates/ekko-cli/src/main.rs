...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Alias { command }) => handle_alias_command(command),

        
        
        Some(Commands::Run { file, path, ekl, root, allow, script_args, snapshot }) =>
            dispatch_run(file, path, ekl, root, allow, script_args, snapshot),

        
        Some(Commands::Doc { topic, selector, llm, interactive, grep }) =>
            doc::handle(topic, selector, llm, interactive, grep),
...
fn normalize_run_argv(mut argv: Vec<String>) -> Vec<String> {
    const SUBS: &[&str] = &[
        ...
        "search", "list", "check", "dev", "x", "init", "ekl", "img", "auth", "alias", "help", "doc",
    ];
    if argv.len() >= 2 {
        let a = &argv[1];
...
async fn run_command(command: Commands) -> anyhow::Result<()> {
    match command {
        ...
        Commands::Init { kind, name_pos, path, name, template, demo } => init_project(kind, path, name.or(name_pos), template, demo).await,
        ...
        Commands::Build { member, all, client, server } => {
            ...
            let _ = ekko_core::packages::store::reconcile_lock_from_store(&cwd);
            ekko_core::packages::workspace::init_workspace(&cwd.join("ekko.json"));
            ...
            if client || (!server && !client) {
              ...
              for proj_member in client_members {
                ...
                use ekko_core::packages::workspace::MemberKind as MK;
                match proj_member.kind {
                    MK::Rune => {} 
                    MK::Web => { build_web(ws)?; continue; }
                    other => {
                        println!("  {} is type \"{}\" — no client build (only `rune` and `web` build a client; use `ekko run` for a `run` project).",
                            proj_member.name, member_kind_label(other));
                        continue;
                    }
                }
                ...
                if entry_dir.exists() {
                    ...
                    if entries.is_empty() {
                        ...
                        if metafile_path.exists() {
                            if let Ok(meta) = serde_json::from_str::<serde_json::Value>(&std::fs::read_to_string(&metafile_path)?) {
                                let mut leaks: std::collections::BTreeMap<String, String> = std::collections::BTreeMap::new();
                                if let Some(outputs) = meta["outputs"].as_object() {
                                    for (_o, info) in outputs {
                                        let entry = info["entryPoint"].as_str().unwrap_or("");
                                        if let Some(imps) = info["imports"].as_array() {
                                            for imp in imps {
                                                let p = imp["path"].as_str().unwrap_or("");
                                                if imp["external"].as_bool().unwrap_or(false) && p.starts_with("ekko:") {
                                                    let raw = if !entry.is_empty() { entry.to_string() } else {
                                                        info["inputs"].as_object()
                                                            .and_then(|m| m.keys().find(|k| k.contains("pages/") || k.contains("components/")).cloned())
                                                            .unwrap_or_default()
                                                    };

                                                    let norm = |s: &str| s.replace('\\', "/").trim_start_matches("//?/").to_string();
                                                    let cleaned = norm(&raw);
                                                    let root_s = norm(&ws.root.to_string_lossy());
                                                    let src = cleaned.strip_prefix(&format!("{}/", root_s)).unwrap_or(&cleaned).to_string();
                                                    leaks.entry(p.to_string()).or_insert(src);
                                                }
                                            }
                                        }
                                    }
                                ...
                                if !leaks.is_empty() {
                                    eprintln!("\n  \x1b[31merror:\x1b[0m a server-only EkkoJS module leaked into the CLIENT bundle:");
                                    for (spec, src) in &leaks {
                                        eprintln!("    \x1b[1m{}\x1b[0m{}", spec, if src.is_empty() { String::new() } else { format!("   (via {})", src) });
                                    }
                                    eprintln!("  cause: these modules run ONLY on the server (database, filesystem, crypto, native APIs) and");
                                    eprintln!("         cannot load in a browser. Imported in client code, the page fails to hydrate (the");
                                    eprintln!("         browser rejects the chunk: CORS / \"module script is text/html\"), so it looks rendered");
                                    eprintln!("         but is dead, no interactivity.");
                                    eprintln!("  fix:   keep server work on the server. Either");
                                    eprintln!("           - move the import + the code using it into an ssr() block wrapped in");
                                    eprintln!("             \x1b[1m/* START SSR */ ... /* END SSR */\x1b[0m  (the client build strips that block), or");
                                    eprintln!("           - fetch the data from an API route (app.api(...)) instead of importing the module in a page.");
                                    eprintln!("         See `ekko doc ru` (Server vs client) and `ekko doc js` (ekko:db).\n");
                                    std::process::exit(1);
                                }
                            }
                        }
        ...
        Commands::Add { packages, build, store, ekl, ekl_path, ekl_deps, ekl_arch, ekl_platform } => {
            ...
            if ekl {
                ...
                if packages.is_empty() { return Err(anyhow::anyhow!("`ekko add --ekl` requires at least one package name")); }
                ...
                let mut queue: Vec<(String, Option<String>)> = packages.iter().map(|p| parse_pkg_spec(p)).collect();
                ...
                while let Some((n, v)) = queue.pop() {
                    ...
                    let is_dep = !packages.iter().any(|p| parse_pkg_spec(p).0 == n);
            ...
            if store {
                if packages.is_empty() { return Err(anyhow::anyhow!("`ekko add -s` requires at least one package name")); }
                use std::io::Write;
                let g = if std::env::var_os("NO_COLOR").is_none() { "\x1b[38;2;61;214;168m" } else { "" };
                let dim = if std::env::var_os("NO_COLOR").is_none() { "\x1b[2m" } else { "" };
                let b = if std::env::var_os("NO_COLOR").is_none() { "\x1b[1m" } else { "" };
                ...
                None => {
                    ...
                    anyhow::bail!(
                        "`ekko add {pkg}` adds a dependency to a workspace member, but the current directory is the workspace root.\n  \
                         ...
                         members: {members}", pkg = pkgs, members = names.join(", "));
                }
            };
            
            let needs_registry = packages.iter().any(|p| !(p.ends_with(".ekl") && std::path::Path::new(p).exists()));
            let client = if needs_registry {
                Some(ekko_core::packages::registry::RegistryClient::from_user_config()?)
            } else { None };
            let mut total = 0usize;
            for package in &packages {
                if package.ends_with(".ekl") && std::path::Path::new(package).exists() {
                    ...
                    let results = ekko_core::packages::store::install_with_deps(client.as_ref().unwrap(), &name, version.as_deref(), category, &target_root)?;
                    for r in &results {
            ...
            if packages.len() > 1 {
                println!("\n{} package(s) added ({} total with dependencies).", packages.len(), total);
            }
            Ok(())
        }
        ...
        Commands::Pack { member, all } => {
            ...
            let pack_and_write = |m: &ekko_core::packages::workspace::Member, dist_root: &std::path::Path| -> anyhow::Result<()> {
                ...
                println!("    \x1b[2mrun it:\x1b[0m ekko run --ekl dist/{}", filename);
                Ok(())
            };
        ...
        Commands::Dev { file, port, allow } => {
            ...
            use ekko_core::packages::workspace::MemberKind as MK;
            match dev_member.kind {
                MK::Rune | MK::Web => {}
                MK::Gui => {
                    let root = ws.root.to_path_buf();
                    return handle_gui_command(GuiCommands::Dev { path: Some(root) });
                }
                other => anyhow::bail!(
                    "`ekko dev` rebuilds a client for rune / web / gui apps; a \"{}\" project has none — run it with `ekko run`.",
                    member_kind_label(other)),
            }
            let is_web = dev_member.kind == MK::Web;
            
            let entry_rel = if is_web {
                dev_member.entry.clone().unwrap_or_else(|| "serve.ts".to_string())
            } else {
                file.to_string_lossy().to_string()
            };
            let server_file = ws.root.join(&entry_rel);
            if !server_file.exists() {
                anyhow::bail!("dev entry not found: {}", server_file.display());
            }

            println!("\x1b[1;35m=== ekko dev ===\x1b[0m \x1b[2m{} ({})\x1b[0m", dev_member.name, member_kind_label(dev_member.kind));
            if enforcing {
            ...
            let dev_build = |w: &ekko_core::packages::workspace::Workspace| -> anyhow::Result<()> {
                if is_web { build_web(w) } else { build_client(w).map(|_| ()) }
            };
            if let Err(e) = dev_build(&ws) {
            ...
            dev_supervisor::install_ctrlc_guard();
            let dev_token = dev_supervisor::dev_token();

            let port_env = port.map(|p| format!("{}", p));
            let mut server_process = dev_supervisor::DevServer::spawn(
                &server_file, port_env.as_deref(), &allow_flags, true, &dev_token)?;
            println!("\x1b[32mServer started (PID {})\x1b[0m", server_process.pid());
                    ...
                    Ok(Ok(event)) => {
                        ...
                        let is_client_change = paths.iter().any(|p| {
                            ...
                            !is_server && (norm.ends_with(".tsx") || norm.ends_with(".ts") || norm.ends_with(".jsx") || norm.ends_with(".js")
                                || norm.ends_with(".scss") || norm.ends_with(".sass") || norm.ends_with(".css") || norm.ends_with(".html"))
                        });
                        ...
                        if is_client_change {
                            ...
                            if is_web {
                                ...
                                match build_web(&ws) {
                                    Ok(()) => println!("\x1b[32mOK\x1b[0m ({}ms)", start.elapsed().as_millis()),
                        ...
                        
                        {
                            ...
                            server_process.stop();
                            match dev_supervisor::DevServer::spawn(&server_file, port_env.as_deref(), &allow_flags, true, &dev_token) {
                                Ok(p) => {
                                    println!("\x1b[32mServer restarted (PID {})\x1b[0m", p.pid());
                                    server_process = p;
                                }
        ...
        Commands::Alias { .. } => unreachable!("alias commands handled in main()"),
    }
}
...
fn handle_alias_command(command: Option<AliasCommands>) -> anyhow::Result<()> {
    use ekko_core::packages::registry::{load_user_config, save_user_config};
    match command.unwrap_or(AliasCommands::List) {
        AliasCommands::List => {
            let cwd = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
            let ws = ekko_core::packages::workspace::run_aliases_from_dir(&cwd);
            let global = load_user_config().map(|c| c.x).unwrap_or_default();
            let print_map = |title: &str, m: &std::collections::HashMap<String, String>| {
                println!("{title}");
                if m.is_empty() { println!("  (none)"); return; }
                let mut keys: Vec<&String> = m.keys().collect();
                keys.sort();
                for k in keys { println!("  {} -> {}", k, m[k]); }
            };
            print_map("workspace aliases (ekko.json \"x\"):", &ws);
            print_map("global aliases (~/.ekko/config.json \"x\"):", &global);
            Ok(())
        }
        AliasCommands::Set { name, target } => {
            if !is_run_alias_key(&name) {
                anyhow::bail!("alias name '{}' must be a plain identifier ([A-Za-z0-9_-], no dots/slashes)", name);
            }
            if target.trim().is_empty() { anyhow::bail!("alias target must not be empty"); }
            let mut cfg = load_user_config()?;
            cfg.x.insert(name.clone(), target.clone());
            save_user_config(&cfg)?;
            println!("set global alias: {} -> {}", name, target);
            Ok(())
        }
        AliasCommands::Rm { name } => {
            let mut cfg = load_user_config()?;
            if cfg.x.remove(&name).is_some() {
                save_user_config(&cfg)?;
                println!("removed global alias: {}", name);
            } else {
                println!("no global alias named '{}'", name);
            }
            Ok(())
        }
    }
...
struct InitTemplate { id: &'static str, label: &'static str, description: &'static str }

fn templates_for(kind: &str) -> &'static [InitTemplate] {
    match kind {
        "rune" => &[
            InitTemplate { id: "showcase", label: "showcase   full-stack tour (SSR + Mimir + multi-page nav)",
                description: "The recommended starting point. A polished, EkkoJS-branded site: server-side \
rendering, Mimir state that survives navigation and a full reload, multi-page client routing with <Link>, \
and a route whose data renders correctly on reload. Hand-styled CSS, no component library." },
            InitTemplate { id: "minimal", label: "minimal    one page, SSR, one atom",
                description: "The smallest real Rune app: a single server-rendered page, one Mimir atom, and \
client navigation. A clean slate to build on or to learn the moving parts." },
            InitTemplate { id: "asgard", label: "asgard     showcase + @ekko/asgard components",
                description: "Everything in showcase, built with the official @ekko/asgard component suite \
(ThemeProvider and themed components) for a ready-made design system." },
            InitTemplate { id: "asgard-minimal", label: "asgard-minimal  one themed page on @ekko/asgard",
                description: "The smallest @ekko/asgard app: one server-rendered page with ThemeProvider + a \
few themed components (Card, TextBox, Button, Checkbox) and a persisted Mimir counter. A clean starting \
point when you want the component library without the full showcase." },
        ],
        "web" => &[
            InitTemplate { id: "static", label: "static   client-only app (HTML + bundled TS)",
                description: "A pure client-side app: index.html + src/main.ts bundled to dist/, served \
statically by ekko:web with an SPA fallback. No SSR, no framework." },
            InitTemplate { id: "api", label: "api      static client + a same-origin JSON API",
                description: "Everything in static, plus an ekko:web server exposing JSON routes under \
/api that the client fetches. Still no SSR." },
        ],
        "gui" => &[
            InitTemplate { id: "app", label: "app    single-window desktop app",
                description: "A minimal asgard-styled desktop window with two-way messaging." },
            InitTemplate { id: "demo", label: "demo   multi-window feature tour",
                description: "The full GUI demo: multi-window, broadcast and direct messaging, a system tray, custom frames." },
        ],
        "tui" => &[
            InitTemplate { id: "docs", label: "docs   offline documentation browser",
                description: "A TUI that reads bundled markdown pages and renders them with the built-in markdown pipeline." },
            InitTemplate { id: "demo", label: "demo   component showcase (one screen per group)",
                description: "A navigable tour of ekko:app/tui: Box/Text, inputs, feedback, Table, code, and a Modal." },
        ],
        _ => &[],
    }
}

fn default_template(kind: &str) -> Option<&'static str> { templates_for(kind).first().map(|t| t.id) }

fn build_templates_json() -> String {
    let mut m = serde_json::Map::new();
    for k in ["run", "lib", "cli", "test", "rune", "web", "gui", "tui", "workspace"] {
        let arr: Vec<serde_json::Value> = templates_for(k).iter()
            .map(|t| serde_json::json!({ "id": t.id, "label": t.label, "description": t.description }))
            .collect();
        m.insert(k.to_string(), serde_json::Value::Array(arr));
    }
    serde_json::Value::Object(m).to_string()
}

async fn init_project(kind: Option<String>, path: Option<PathBuf>, name: Option<String>,
                      template: Option<String>, demo: bool) -> anyhow::Result<()> {
    use std::io::IsTerminal;
    let cwd = std::env::current_dir().unwrap_or_default();

    let (path, name) = if name.as_deref() == Some(".") { (Some(cwd.clone()), None) } else { (path, name) };
    ...
    let template = template.or_else(|| if demo { Some("demo".to_string()) } else { None });

    
    let kind_has_variants = kind.as_deref().map(|k| templates_for(k).len() > 1).unwrap_or(false);
    let need_picker = kind.is_none()
        || (kind_has_variants && template.is_none() && std::io::stdin().is_terminal());

    let (chosen_kind, chosen_template, chosen_name) = if need_picker {
        ...
        unsafe {
            ...
            std::env::set_var("EKKO_INIT_TEMPLATES", build_templates_json());

            match kind.as_deref() {
                Some(k) => { std::env::set_var("EKKO_INIT_KIND", k);
                             std::env::set_var("EKKO_INIT_NAME", name.as_deref().unwrap_or(dir_name.as_str())); }
                None    => { std::env::remove_var("EKKO_INIT_KIND"); std::env::remove_var("EKKO_INIT_NAME"); }
            }
            if let Some(t) = template.as_deref() { std::env::set_var("EKKO_INIT_TEMPLATE", t); }
            else { std::env::remove_var("EKKO_INIT_TEMPLATE"); }
            std::env::remove_var("EKKO_INIT_RESULT");
        }

        let picker_js = ekko_core::parsers::swc_transform::transpile(
            include_str!("scaffold/init-picker.ts"), "init-picker.ts")?;
        ekko_core::engine::v8_runtime::execute_module_async(&picker_js, "init-picker").await?;
        ...
        let t = v.get("template").and_then(|t| t.as_str()).filter(|s| !s.is_empty()).map(|s| s.to_string());
        ...
        (kind.unwrap(), template, name.unwrap_or_else(|| dir_name.clone()))
    };

    let tmpls = templates_for(&chosen_kind);
    let tmpl = if tmpls.is_empty() {
        String::new()
    } else {
        let t = chosen_template.unwrap_or_else(|| default_template(&chosen_kind).unwrap().to_string());
        if !tmpls.iter().any(|d| d.id == t) {
            anyhow::bail!("unknown template '{}' for '{}'. Valid templates: {}.",
                t, chosen_kind, tmpls.iter().map(|d| d.id).collect::<Vec<_>>().join(", "));
        }
        t
    };
    ...
    match chosen_kind.as_str() {
        ...
        "rune" => scaffold_rune(&target, &chosen_name, &tmpl)?,
        "web"  => scaffold_web(&target, &chosen_name, &tmpl)?,
        "gui"  => if tmpl == "demo" { gui_init_demo(&target, &chosen_name)? } else { gui_init(&target, &chosen_name)? },

        "tui"  => if tmpl == "demo" { tui_init_demo(&target, &chosen_name)? } else { tui_init(&target, &chosen_name)? },
        other  => anyhow::bail!(
            "unknown project type '{}'. Valid types: run, lib, cli, test, rune, web, gui, tui, workspace.\n\
             Use `ekko init <type> [-t <template>]`, or `ekko init` for an interactive picker.",
            other),
    }
...
fn scaffold_web(target: &std::path::Path, name: &str, template: &str) -> anyhow::Result<()> {
    let api = template == "api";
    println!("\x1b[1;35mScaffolding web ({}) project: {}\x1b[0m\n", template, name);
    let entry = if api { "server.ts" } else { "serve.ts" };
    scaffold_write(&target.join("ekko.json"), &format!(
        "{{\n  \"name\": \"{}\",\n  \"version\": \"1.0.0\",\n  \"type\": \"web\",\n  \"entry\": \"{}\",\n  \
\"build\": {{ \"@ekko/esbuild\": \"*\" }},\n  \"permissions\": {{ \"net\": true, \"fs\": true }},\n  \
\"package\": {{ \"include\": [\"dist/**/*\", \"static/**/*\", \"index.html\"] }}\n}}\n",
        name, entry))?;
    scaffold_write(&target.join("index.html"), include_str!("scaffold/web/index.html"))?;
    scaffold_write(&target.join("styles.css"), include_str!("scaffold/web/styles.css"))?;
    scaffold_write(&target.join("src/util.ts"), include_str!("scaffold/web/util.ts"))?;
    scaffold_write(&target.join("src/main.ts"),
        if api { include_str!("scaffold/web/main-api.ts") } else { include_str!("scaffold/web/main.ts") })?;
    scaffold_write(&target.join(entry),
        if api { include_str!("scaffold/web/server.ts") } else { include_str!("scaffold/web/serve.ts") })?;
    println!("\n\x1b[32mDone!\x1b[0m Install, build, then run:");
    println!("  \x1b[1mekko add\x1b[0m                  \x1b[2m# fetch the build toolchain into the store\x1b[0m");
    println!("  \x1b[1mekko build\x1b[0m                \x1b[2m# bundle src/main.ts -> dist/\x1b[0m");
    println!("  \x1b[1mekko run {} --allow=net,fs\x1b[0m", entry);
    println!("Then open \x1b[1mhttp://localhost:3000\x1b[0m.");
    Ok(())
}

fn scaffold_rune(target: &std::path::Path, name: &str, template: &str) -> anyhow::Result<()> {
    let with_asgard = template == "asgard" || template == "asgard-minimal";
    let heavy = template == "showcase" || template == "asgard";   
    println!("\x1b[1;35mScaffolding rune ({}) project: {}\x1b[0m\n", template, name);

    
    let thread = |s: &str| s.replace("Rune App", name);

    let asgard_ship = if with_asgard { "\n    \"@ekko/asgard\": \"^1.0.0\"," } else { "" };
    scaffold_write(&target.join("ekko.json"), &format!(
        "{{\n  \"name\": \"{}\",\n  \"version\": \"1.0.0\",\n  \"type\": \"rune\",\n  \"entry\": \"server.tsx\",\n  \
...
\"@ekko/react-dom\": \"^19.0.0\"\n  }},\n  \"permissions\": {{ \"net\": true, \"fs\": true, \"env\": true, \"crypto\": true }},\n  \
\"package\": {{ \"include\": [\"pages/**/*\", \"components/**/*\", \"atoms/**/*\", \"lib/**/*\", \"styles/**/*\", \
\"static/**/*\", \"content/**/*\", \".ekko/build/**/*\"] }}\n}}\n",
        name, asgard_ship))?;

    
    let server = if template == "minimal" { include_str!("scaffold/rune/server-minimal.tsx") }
                 else { include_str!("scaffold/rune/server.tsx") };
    scaffold_write(&target.join("server.tsx"), &thread(server))?;

    let layout = match template {
        "asgard-minimal" => include_str!("scaffold/rune/pages/layout-asgard-minimal.tsx"),
        "minimal"        => include_str!("scaffold/rune/pages/layout-minimal.tsx"),
        _                => include_str!("scaffold/rune/pages/layout-asgard.tsx"),
    };
    scaffold_write(&target.join("pages/layout.tsx"), &thread(layout))?;
    
    let index = match template {
        "asgard"         => include_str!("scaffold/rune/pages/index-asgard.tsx"),
        "asgard-minimal" => include_str!("scaffold/rune/pages/index-asgard-minimal.tsx"),
        "showcase"       => include_str!("scaffold/rune/pages/index.tsx"),
        _                => include_str!("scaffold/rune/pages/index-minimal.tsx"),
    };
    scaffold_write(&target.join("pages/index.tsx"), &thread(index))?;
    scaffold_write(&target.join("pages/not-found.tsx"), include_str!("scaffold/rune/pages/not-found.tsx"))?;
    scaffold_write(&target.join("pages/error.tsx"), include_str!("scaffold/rune/pages/error.tsx"))?;

    let atoms = if heavy { include_str!("scaffold/rune/atoms/app.ts") }
                else { include_str!("scaffold/rune/atoms/app-minimal.ts") };
    scaffold_write(&target.join("atoms/app.ts"), &thread(atoms))?;
    scaffold_write(&target.join("styles/global.scss"), include_str!("scaffold/rune/styles/global.scss"))?;
    scaffold_write(&target.join("static/ekko-icon-rounded.svg"), include_str!("scaffold/rune/static/ekko-icon-rounded.svg"))?;

    println!("\n\x1b[32mDone!\x1b[0m Install the dependencies, build the client, then run:");
    println!("  \x1b[1mekko add\x1b[0m                  \x1b[2m# fetch @ekko/react + @ekko/react-dom into the store\x1b[0m");
    println!("  \x1b[1mekko build --client\x1b[0m       \x1b[2m# generate the client bundles (.ekko/build)\x1b[0m");
    println!("  \x1b[1mekko run server.tsx --allow=fs,net,env,crypto\x1b[0m   \x1b[2m(or `ekko dev server.tsx` for hot reload)\x1b[0m");
    println!("Then open \x1b[1mhttp://localhost:3000\x1b[0m.");
    Ok(())
}
...
fn scaffold_tool(target: &std::path::Path, name: &str) -> anyhow::Result<()> {
    ...
    println!("\n\x1b[32mDone!\x1b[0m Open the interactive menu with \x1b[1mekko x {}\x1b[0m, or jump straight to a demo: \x1b[1mekko x {} show colors\x1b[0m.", name, name);
    Ok(())
}
...
#[cfg(not(feature = "gui"))]
fn handle_gui_command(_command: GuiCommands) -> anyhow::Result<()> {
    anyhow::bail!("GUI apps require the EkkoJS desktop build — this is the headless (server) build. Install the desktop build to run GUI apps.")
}
...
fn handle_tui_command(command: TuiCommands, snapshot: bool) -> anyhow::Result<()> {
    match command {
        TuiCommands::Run { package, path, ekl } => {
            if let Some(ekl_path) = ekl {
                apply_workspace_permissions_from_dir(&std::env::current_dir().unwrap_or_default());
                run_tui_from_ekl(&ekl_path, snapshot)
            } else if let Some(ref pkg_name) = package {
                if std::path::Path::new(pkg_name).is_dir() {
                    let (project_dir, entry) = resolve_tui_project(Some(PathBuf::from(pkg_name)))?;
                    apply_workspace_permissions_from_dir(&project_dir);
                    let title = read_tui_name(&project_dir);
                    ekko_tui::run(project_dir.join(&entry), title, snapshot)
                } else {
                    apply_workspace_permissions_from_dir(&std::env::current_dir().unwrap_or_default());
                    let (store_dir, entry, title) = resolve_store_package(pkg_name, "tui")?;
                    ekko_tui::run(store_dir.join(&entry), title, snapshot)
                }
            } else {
...
fn tui_init_demo(target: &std::path::Path, name: &str) -> anyhow::Result<()> {
    ...
    write_if_missing(&target.join("app.tsx"), &include_str!("scaffold/tui-demo/app.tsx").replace("{{NAME}}", name))?;
...
fn member_kind_label(k: ekko_core::packages::workspace::MemberKind) -> &'static str {
    use ekko_core::packages::workspace::MemberKind as MK;
    match k {
        MK::Run => "run", MK::Rune => "rune", MK::Web => "web", MK::Lib => "lib",
        MK::Tool => "tool", MK::Test => "test", MK::Gui => "gui", MK::Tui => "tui",
    }
}

fn build_web(ws: &ekko_core::packages::workspace::Workspace) -> anyhow::Result<()> {
    let src_dir = ws.root.join("src");
    let entry = ["main.ts", "main.tsx", "main.js", "main.jsx", "index.ts", "index.tsx"]
        .iter().map(|n| src_dir.join(n)).find(|p| p.exists());
    let entry = match entry {
        Some(e) => e,
        None => {
            println!("  no client entry found — a `web` app needs `src/main.ts` (or .tsx/.js).");
            return Ok(());
        }
    };
    let has_jsx = matches!(entry.extension().and_then(|e| e.to_str()), Some("tsx") | Some("jsx"));

    let dist = ws.root.join("dist");
    let _ = std::fs::remove_dir_all(&dist); 
    std::fs::create_dir_all(&dist)?;
    let esbuild = ensure_esbuild_available()?;
    println!("Building web bundle for {}...", entry.file_name().map(|f| f.to_string_lossy().into_owned()).unwrap_or_default());
    ...
    let jsx_file = ws.root.join("__jsx.js");
    if has_jsx {
        std::fs::write(&jsx_file, get_jsx_client_js())?;
        cmd.arg(format!("--alias:ekko:jsx-runtime={}", jsx_file.display()));
        cmd.arg(format!("--alias:ekko/jsx-runtime={}", jsx_file.display()));
        cmd.args(["--jsx=automatic", "--jsx-import-source=ekko"]);
    }

    cmd.args([
        "--bundle", "--format=esm", "--platform=browser",
        "--entry-names=[name]",
        &format!("--outdir={}", dist.display()),
        "--external:ekko:*",
        "--define:process.env.NODE_ENV=\"production\"",
        "--target=es2022,chrome90,firefox90,safari15",
        "--minify",
    ...
    let output = cmd.output()?;
    if has_jsx { let _ = std::fs::remove_file(&jsx_file); }
    if !output.status.success() {
        eprintln!("{}", String::from_utf8_lossy(&output.stderr));
    ...
    let index = ws.root.join("index.html");
    if index.exists() { std::fs::copy(&index, dist.join("index.html"))?; }
    let static_dir = ws.root.join("static");
    if static_dir.exists() { copy_dir_all(&static_dir, &dist.join("static"))?; }
    for e in std::fs::read_dir(&ws.root).into_iter().flatten().flatten() {
        let p = e.path();
        if p.extension().and_then(|x| x.to_str()) == Some("css") {
            if let Some(fname) = p.file_name() { let _ = std::fs::copy(&p, dist.join(fname)); }
        }
    }

    println!("  \u{2192} web bundle: {}", dist.display());
    Ok(())
}
...
// HMR: WebSocket live reload — DEV ONLY. The server injects `window.__ekkoDev` (into <head>) and
// registers /__ekko_hmr ONLY under `ekko dev` (EKKO_DEV=1). In production neither exists, so this is
// inert: no WebSocket, no reconnect loop, no console noise. (m36 task-3a)
(function() {
  if (!window.__ekkoDev) return;
...
// Hard load: seed the current route params from the SSR payload so useParams() works on first render
// (not just after a client nav). Without this _currentParams stayed empty and useParams() returned nothing.
if (data.props && data.props.params) {{ _currentParams = data.props.params; window.__ekko_router_params = _currentParams; if (window.__ekko_router_set_params) window.__ekko_router_set_params(_currentParams); }}
...
fn dispatch_run(
    ...
    root: Option<PathBuf>, allow: Vec<String>, _script_args: Vec<String>, snapshot: bool,
) -> anyhow::Result<()> {
    ...
    if snapshot {
        if let RunTarget::Module(RunSource::DiskFile(p)) = &target {
            let dir = p.parent().map(|d| d.to_path_buf())
                .unwrap_or_else(|| std::env::current_dir().unwrap_or_default());
            apply_workspace_permissions_from_dir(&dir);
            let title = read_tui_name(&dir);
            return ekko_tui::run(p.clone(), title, true);
        }
    }
    match target {
        
        RunTarget::Gui { package, path, ekl } => handle_gui_command(GuiCommands::Run { package, path, ekl }),
        RunTarget::Tui { package, path, ekl } => handle_tui_command(TuiCommands::Run { package, path, ekl }, snapshot),
        RunTarget::Lib(name) => {
...
fn is_run_alias_key(s: &str) -> bool {
    !s.is_empty() && s.bytes().all(|b| b.is_ascii_alphanumeric() || b == b'-' || b == b'_')
}

fn expand_run_alias(
    file: Option<&std::path::Path>, path: Option<&std::path::Path>, ekl: Option<&std::path::Path>,
) -> anyhow::Result<(Option<std::path::PathBuf>, Option<(String, String)>)> {

    if path.is_some() || ekl.is_some() { return Ok((file.map(|f| f.to_path_buf()), None)); }
    let f = match file { Some(f) => f, None => return Ok((None, None)) };
    let raw = f.to_string_lossy().to_string();
    if !is_run_alias_key(&raw) { return Ok((Some(f.to_path_buf()), None)); }
    let cwd = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
    let mut global: Option<std::collections::HashMap<String, String>> = None;
    let mut global_loaded = false;
    let mut cur = raw.clone();
    let mut expanded = false;

    let mut anchor: Option<std::path::PathBuf> = None;
    let mut seen = std::collections::HashSet::new();
    while is_run_alias_key(&cur) {

        let next = match ekko_core::packages::workspace::lookup_run_alias(&cwd, &cur) {
            Some((v, dir)) => { anchor = Some(dir); Some(v) }
            None => {
                if !global_loaded {
                    global = ekko_core::packages::registry::load_user_config().ok().map(|c| c.x);
                    global_loaded = true;
                }
                match global.as_ref().and_then(|m| m.get(&cur).cloned()) {
                    Some(v) => { anchor = None; Some(v) }
                    None => None,
                }
            }
        };
        match next {
            Some(v) => {
                if !seen.insert(cur.clone()) { anyhow::bail!("`ekko x`: alias cycle resolving '{}'", raw); }
                if seen.len() > 16 { anyhow::bail!("`ekko x`: alias chain too deep resolving '{}'", raw); }
                cur = v;
                expanded = true;
            }
            None => break, 
    ...
    let applied = if expanded { Some((raw.clone(), cur.clone())) } else { None };

    if let Some(dir) = &anchor {
        if cur.starts_with("./") || cur.starts_with("../") || cur.starts_with(".\\") || cur.starts_with("..\\") {
            return Ok((Some(dir.join(&cur)), applied));
        }
    }
    Ok((Some(std::path::PathBuf::from(cur)), applied))
}
...
fn resolve_run_target(
    ...
    let (expanded, alias_applied) = expand_run_alias(file, path, ekl)?;
    let file = expanded.as_deref();
    ...
    let _ = ekko_core::packages::store::reconcile_lock_from_store(&root);
    ...
    if let Some(f) = file {
        ...
        if let Some((alias, tgt)) = &alias_applied {
            anyhow::bail!("alias '{}' -> '{}' could not be resolved: not a store package, a file, or a known command", alias, tgt);
        }
    ...
    match m.kind {
        ...
        MemberKind::Run | MemberKind::Rune | MemberKind::Web => {
            let e = m.entry.as_deref().ok_or_else(|| anyhow::anyhow!(
                "project '{}' (type \"{}\") declares no \"entry\" in ekko.json",
                m.name,
                match m.kind { MemberKind::Rune => "rune", MemberKind::Web => "web", _ => "run" }))?;
    ...
    match ptype {
        "run" | "rune" | "web" => {
            let entry = manifest.get("entry").and_then(|e| e.as_str())
                .ok_or_else(|| anyhow::anyhow!("store package '{}' ({}) has no entry", name, ptype))?;
            let p = stored(entry);
...
fn run_tui_from_ekl(ekl_path: &std::path::Path, snapshot: bool) -> anyhow::Result<()> {
    let pkg = ekko_vfs::EklPackage::from_file(ekl_path)
        .map_err(|e| anyhow::anyhow!("failed to load .ekl: {}", e))?;
    let entry = pkg.metadata.entry.as_deref()
        .ok_or_else(|| anyhow::anyhow!("no entry in .ekl metadata, package type {:?}", pkg.metadata.project_type))?;
    ...
    let title = pkg.metadata.name.clone();
    let display = format!("ekl:///{}/{}", pkg.metadata.name, entry);
    ekko_tui::run_from_vfs((*source).clone(), display, title, snapshot)
}
...
mod run_resolution_tests {
    ...
    fn normalize_subs_covers_every_clap_subcommand() {
        ...
        const SUBS: &[&str] = &[
            "run", "eval", "repl", "test", "build", "pack", "add", "remove", "publish",
            "search", "list", "check", "dev", "x", "init", "gui", "tui", "ekl", "img", "auth", "alias", "help", "doc",
        ];
        let cmd = Cli::command();
