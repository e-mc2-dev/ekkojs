...
async fn run_command(command: Commands) -> anyhow::Result<()> {
    match command {
        ...
        Commands::Add { package, build, store } => {
            ...
            if store {
                ...
                let (root_name, root_ver) = match package.split_once('@').filter(|(n, _)| !n.is_empty()) {
                    Some((n, v)) => (n.to_string(), Some(v.to_string())),
                    None => (package.clone(), None),
                };
                ...
                while let Some((n, v)) = queue.pop() {
                    ...
                    let is_dep = count > 0;
                    let label = if is_dep { format!("dep: downloading {}", n) } else { format!("downloading {}", n) };
                    ...
                    let tag = if is_dep { format!("{dim}dep{r} ") } else { String::new() };
                    println!("  {g}\u{2713}{r} {tag}{b}{}{r}{dim}@{} ({:.1} KB){r}", result.name, result.version, kb);
                    count += 1;
                    for dep in ekko_core::packages::store::ship_deps_of(&result.store_path) {
                        if !seen.contains(&dep) { queue.push((dep, None)); }
                    }
                    let _ = std::io::stdout().flush();
                }
                println!("\n  {g}\u{2713} done{r} {dim}: {} package(s) in {}{r}\n", count, ekko_core::packages::store::store_root().display());
                return Ok(());
            }
            let cwd = std::env::current_dir().unwrap_or_default();
            ekko_core::packages::workspace::init_workspace(&cwd.join("ekko.json"));
            ...
            if package.ends_with(".ekl") && std::path::Path::new(&package).exists() {
                let result = ekko_core::packages::store::install_local(std::path::Path::new(&package), category, &ws.root)?;
                println!("+ {}@{} (local, {} bytes)", result.name, result.version, result.size);
            } else {
                let (name, version) = if let Some((n, v)) = package.split_once('@').filter(|(n, _)| !n.is_empty()) {
                    (n.to_string(), Some(v))
                } else {
                    (package.clone(), None)
                };
                let client = ekko_core::packages::registry::RegistryClient::from_user_config()?;
                let results = ekko_core::packages::store::install_with_deps(&client, &name, version, category, &ws.root)?;
                for r in &results {
        ...
        Commands::Dev { file, port } => {
            ...
            println!("\x1b[1;35m=== ekko dev ===\x1b[0m");
            ...
            let mut server_process = start_dev_server(&server_file, port_env.as_deref())?;
                    ...
                    Ok(Ok(event)) => {
                        ...
                        if is_client_change {
                            ...
                            match build_client(&ws) {
                                Ok(_) => println!("\x1b[32mOK\x1b[0m ({}ms)", start.elapsed().as_millis()),
                        ...
                        
                        {
                            ...
                            match start_dev_server(&server_file, port_env.as_deref()) {
                                Ok(p) => {
...
fn build_client(ws: &ekko_core::packages::workspace::Workspace) -> anyhow::Result<()> {
    let pages_dir = ws.root.join("pages");
    let app_dir = ws.root.join("app");
    let entry_dir = if pages_dir.exists() { pages_dir } else if app_dir.exists() { app_dir } else { ws.root.join("src") };
    if !entry_dir.exists() { return Ok(()); }
            ...
            if let Some(ext) = p.extension().and_then(|e| e.to_str()) {
                if ext == "scss" || ext == "sass" {
                    ...
                    match ekko_core::parsers::css::compile_sass(&src, syntax) {
                        Ok(css) => {
                            let rel = p.strip_prefix(&styles_dir).unwrap_or(p).to_string_lossy().to_string();
...
fn start_dev_server(file: &std::path::Path, port: Option<&str>) -> anyhow::Result<std::process::Child> {
...
// Link component — renders <a data-nav> for client-side navigation
window.Link = function Link(props) {{
  const {{ href, children, className, ...rest }} = props || {{}};
  return createElement("a", {{ ...rest, href, className, "data-nav": "", "data-ekko-prefetch": "true", children }});
}};
...
async fn run_module(
    ...
    } else if let Some(perm_json) = ekko_core::packages::workspace::get_workspace().and_then(|ws| ws.permissions.as_ref()) {
