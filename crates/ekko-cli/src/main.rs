...
fn scaffold_rune(target: &std::path::Path, name: &str, template: &str) -> anyhow::Result<()> {
    let with_asgard = template == "asgard";
    let full = template != "minimal";    
    println!("\x1b[1;35mScaffolding rune ({}) project: {}\x1b[0m\n", template, name);

    
    let asgard_ship = if full { "\n    \"@ekko/asgard\": \"1.0.5\"," } else { "" };
    scaffold_write(&target.join("ekko.json"), &format!(
        "{{\n  \"name\": \"{}\",\n  \"version\": \"1.0.0\",\n  \"type\": \"rune\",\n  \"entry\": \"server.tsx\",\n  \
...
\"static/**/*\", \"content/**/*\", \".ekko/build/**/*\"] }}\n}}\n",
    ...
    scaffold_write(&target.join("server.tsx"),
        if full { include_str!("scaffold/rune/server.tsx") } else { include_str!("scaffold/rune/server-minimal.tsx") })?;

    scaffold_write(&target.join("pages/layout.tsx"),
        if full { include_str!("scaffold/rune/pages/layout-asgard.tsx") }
        else { include_str!("scaffold/rune/pages/layout-minimal.tsx") })?;
    scaffold_write(&target.join("pages/index.tsx"),
        if with_asgard { include_str!("scaffold/rune/pages/index-asgard.tsx") }
        else if full { include_str!("scaffold/rune/pages/index.tsx") }
        else { include_str!("scaffold/rune/pages/index-minimal.tsx") })?;
    ...
    scaffold_write(&target.join("atoms/app.ts"), include_str!("scaffold/rune/atoms/app.ts"))?;
    ...
    println!("  \x1b[1mekko run server.tsx --allow=fs,net\x1b[0m   \x1b[2m(or `ekko dev server.tsx` for hot reload)\x1b[0m");
    println!("Then open \x1b[1mhttp://localhost:3000\x1b[0m.");
    Ok(())
}
...
mod run_resolution_tests {
    ...
    fn normalize_subs_covers_every_clap_subcommand() {
        ...
        const SUBS: &[&str] = &[
            "run", "eval", "repl", "test", "build", "pack", "add", "remove", "publish",
            "search", "list", "check", "dev", "x", "init", "gui", "tui", "ekl", "img", "auth", "help", "doc",
        ];
        let cmd = Cli::command();
