...
pub fn handle(
    ...
    if let Some(pattern) = grep.as_deref() {
        use std::io::IsTerminal;
        let color = std::env::var_os("NO_COLOR").is_none() && std::io::stdout().is_terminal();
        return search(&bundle, scope_of(&req), pattern, color);
    }
    ...
    let color = std::env::var_os("NO_COLOR").is_none() && is_tty;
    ...
    match req {
        Request::Intro => {
            if llm {
                ...
                print!("{}", md_ansi::render(&bundle.intro, term_width(), color));
            }
        }
...
fn scope_of(req: &Request) -> Scope {
    match req {
        Request::Intro => Scope::All,
        Request::Tree(k) => Scope::Topic(k.clone()),
        Request::Page(k) => Scope::Page(k.clone()),
    }
}

const GREP_CONTEXT: usize = 10;

const MAX_RESULT_PAGES: usize = 40;

fn search(b: &Bundle, scope: Scope, pattern: &str, color: bool) -> Result<()> {
    
    let smart_ci = !pattern.chars().any(|c| c.is_ascii_uppercase());
    let re = regex::RegexBuilder::new(pattern)
        .case_insensitive(smart_ci)
        .build()
        .map_err(|e| anyhow!("invalid --grep pattern: {e}"))?;

    let topic_rank = |key: &str| -> usize {
        let t = key.split('/').next().unwrap_or("");
        b.topics.iter().position(|x| x.key == t).unwrap_or(usize::MAX)
    };
    let mut keys: Vec<&String> = b
        ...
        .filter(|k| match &scope {
            Scope::All => true,
            Scope::Topic(t) => k.split('/').next() == Some(t.as_str()),
            Scope::Page(p) => k.as_str() == p.as_str(),
        })
        .collect();
    keys.sort_by(|a, b2| topic_rank(a).cmp(&topic_rank(b2)).then(a.as_str().cmp(b2.as_str())));

    
    
    let mut matches: Vec<(&String, Vec<(usize, usize)>)> = Vec::new();
    for key in keys {
        let windows = snippet_windows(&b.docs[key].raw, &re, GREP_CONTEXT);
        if !windows.is_empty() {
            matches.push((key, windows));
        }
    }

    let where_ = || match &scope {
        Scope::All => "the docs".to_string(),
        Scope::Topic(t) => format!("topic '{t}'"),
        Scope::Page(p) => format!("page {p}"),
    };

    
    if matches.is_empty() {
        println!("no matches for /{pattern}/ in {}.", where_());
        println!("Tip: relax the pattern (regex), or widen the scope by dropping the topic/page. `ekko doc` lists topics.");
        return Ok(());
    }

    let shown = matches.len().min(MAX_RESULT_PAGES);
    for (key, windows) in &matches[..shown] {
        let d = &b.docs[*key];
        print!("{}", result_header(key, d, color));
        ...
        for (i, (s, e)) in windows.iter().enumerate() {
            ...
            for ln in *s..=*e {
                println!("{}", highlight_line(lines[ln], &re, color));
            }
        }
        println!();
    }
    ...
    if matches.len() > shown {
        ...
        let msg = format!(
            "... and {more} more matching page(s) in {}. Narrow the pattern (regex), or scope it: `ekko doc <topic> [<NNN>] --grep …`.",
            where_()
        );
        if color {
...
fn result_header(key: &str, d: &Doc, color: bool) -> String {
    let (topic, num) = key.split_once('/').unwrap_or((key, ""));
    let cmd = format!("ekko doc {topic} {num}");
    let crumbs = d.crumbs.join(" › ");
    if color {
        format!(
            "\n\x1b[1;36m--- {cmd}\x1b[0m  \x1b[90m·  {crumbs}  ·  {}\x1b[0m \x1b[1;36m---\x1b[0m\n",
            d.url
        )
    } else {
        format!("\n--- {cmd}  ·  {crumbs}  ·  {} ---\n", d.url)
...
fn highlight_line(line: &str, re: &regex::Regex, color: bool) -> String {
    if !color {
        return line.to_string();
    }
    re.replace_all(line, "\x1b[1;33m$0\x1b[0m").into_owned() 
}

fn snippet_windows(text: &str, re: &regex::Regex, ctx: usize) -> Vec<(usize, usize)> {
    ...
    let para_start = |m: usize| {
        let mut i = m;
        while i > 0 && !blank(lines[i - 1]) {
            i -= 1;
        }
        i
    };
    let para_end = |m: usize| {
        let mut i = m;
        while i < last && !blank(lines[i + 1]) {
            i += 1;
        }
        i
    };
    ...
    for (m, line) in lines.iter().enumerate() {
        ...
        let ps = para_start(m);
        let pe = para_end(m);
        let lo = ps.min(m.saturating_sub(ctx)); 
        let hi = pe.max((m + ctx).min(last)); 
        wins.push((para_start(lo), para_end(hi))); 
    }
    ...
    for (s, e) in wins {
        match merged.last_mut() {
            Some(prev) if s <= prev.1 + 1 => prev.1 = prev.1.max(e),
            _ => merged.push((s, e)),
        }
    }
    merged
...
fn llm_reminder() -> String {
    "-= Reminder =-\n\
     ekko doc --llm is the only source of truth for EkkoJS APIs.\n\
     Run `ekko doc <topic> <NNN> --llm` before writing code that uses anything you do not already have the doc for.\n\
     Find it fast first: `ekko doc [<topic> [<NNN>]] --grep \"<pattern>\"`  (no args = all topics; <topic> = that topic; <topic> <NNN> = that one page; regex + smart-case).\n\
     -= End =-\n\n"
        .to_string()
}

fn ai_guidance() -> String {
    "-= Ekko AI live guidance =-\n\
     This section is displayed only on `ekko doc --llm`.\n\
     EkkoJS is an AI-guided JS/TS runtime: consult it before each round of coding or analysis so you have the right plan, the global architecture picture, and the correct syntax.\n\
     This live guidance is the source of truth and requires no prior knowledge.\n\
     EkkoJS is NOT Node, Deno, or Bun. It has its own API, patterns, and syntax.\n\
     -= end guidance =-\n\n"
        .to_string()
}
...
fn launch_interactive() -> Result<()> {
    ...
    ekko_tui::run_from_vfs((*src).clone(), format!("ekl:///{name}/{entry}"), name, false)
}
...
mod tests {
    ...
    #[test]
    fn scope_of_maps_request() {
        assert!(matches!(scope_of(&Request::Intro), Scope::All));
        assert!(matches!(scope_of(&Request::Tree("js".into())), Scope::Topic(ref t) if t == "js"));
        assert!(matches!(scope_of(&Request::Page("js/001".into())), Scope::Page(ref p) if p == "js/001"));
    }

    #[test]
    fn smart_case_logic() {
        
        let ci = |p: &str| !p.chars().any(|c| c.is_ascii_uppercase());
        assert!(ci("permission"));
        assert!(!ci("Permission"));
        assert!(!ci("API"));
    }

    #[test]
    fn snippet_windows_paragraph_and_context() {
        let re = regex::Regex::new("needle").unwrap();
        
        let mut t = String::new();
        for i in 0..30 {
            if i == 16 {
                t.push_str("has a needle in it\n");
            } else if i % 5 == 0 {
                t.push('\n');
            } else {
        ...
        let w = snippet_windows(&t, &re, 3);
        assert_eq!(w.len(), 1, "one window for one match");
        let (s, e) = w[0];
        assert!(s <= 16 && e >= 16, "window straddles the match line");
        assert!(e - s >= 3, "at least the requested context spans");
        assert!(s == 0 || t.lines().nth(s - 1).unwrap().trim().is_empty(), "start snapped to a paragraph boundary");
    }

    #[test]
    fn snippet_windows_merges_nearby_matches() {
        let re = regex::Regex::new("x").unwrap();
        let t = "a x b\nc\nd\ne x f\n";
        assert_eq!(snippet_windows(t, &re, 10).len(), 1, "nearby matches merge");
    }

    #[test]
    fn snippet_windows_no_match_is_empty() {
        let re = regex::Regex::new("zzz").unwrap();
        assert!(snippet_windows("hello\nworld\n", &re, 10).is_empty());
    }

    #[test]
    fn result_header_names_the_open_command() {
        let d = doc("js", "042");
        let h = result_header("js/042", &d, false);
        assert!(h.contains("--- ekko doc js 042"), "names the exact open command: {h:?}");
        assert!(h.contains(&d.url));
        assert!(h.contains("Docs")); 
        assert!(!h.contains('\x1b'), "plain header has no ANSI");
    }

    #[test]
    fn highlight_only_on_color() {
        let re = regex::Regex::new("hit").unwrap();
        assert_eq!(highlight_line("a hit b", &re, false), "a hit b");
        assert!(highlight_line("a hit b", &re, true).contains("\x1b[1;33mhit\x1b[0m"));
    }

    #[test]
    fn llm_reminder_has_markers_and_points_at_grep() {
        let r = llm_reminder();
        assert!(r.starts_with("-= Reminder =-"));
        assert!(r.contains("-= End =-"));
        assert!(r.contains("source of truth"));
        assert!(r.contains("--grep"));
        assert!(!r.contains('\x1b'), "reminder is plain text");
    }

    #[test]
    fn ai_guidance_has_markers_and_positions_ekko() {
        let g = ai_guidance();
        assert!(g.starts_with("-= Ekko AI live guidance =-"));
        assert!(g.contains("-= end guidance =-"));
        assert!(g.contains("source of truth"));
        assert!(g.contains("NOT Node, Deno, or Bun"));
        assert!(!g.contains('\x1b'), "guidance is plain text");
    }
}
