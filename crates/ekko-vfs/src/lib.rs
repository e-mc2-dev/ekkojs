...
impl EklHeader {
    ...
    fn parse(data: &[u8]) -> anyhow::Result<Self> {
        ...
        if &data[0..4] != MAGIC {
            ...
            let head = String::from_utf8_lossy(&data[0..4.min(data.len())]);
            if data.starts_with(b"<!") || data.starts_with(b"<ht") || data.starts_with(b"<HT") {
                anyhow::bail!(
                    "this is not an .ekl — it looks like an HTML file (starts with \"{}\"). The runnable \
                     package is the `*.any.any.ekl` that `ekko pack` produces (in the dist/ folder), not the \
                     build output. Try: ekko run --ekl dist/<name>.any.any.ekl", head);
            }
            anyhow::bail!(
                "not a valid .ekl: expected the 'EKKO' header but got {:?} (\"{}\"). Pass the `*.any.any.ekl` \
                 produced by `ekko pack`, not a source or build file.", &data[0..4], head);
        }
...
mod tests {
    ...
    fn header_magic_validation() {
        ...
        assert!(err.to_string().contains("not a valid .ekl"));
        assert!(err.to_string().contains("EKKO"));
    }
