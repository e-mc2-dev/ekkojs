...
public static class WebApiExports
{
    ...
    public static void EkkoWebFetch(NativeString url, NativeString method, NativeString headersJson, NativeBuffer body, NativeString redirect, nint callbackPtr, long context)
    {
        _ = Task.Run(async () =>
        {
            NativeResult result;
            try
            {
                var r = await WebApi.Fetch(url.ToManaged(), method.ToManaged(), headersJson.ToManaged(), body.ToArray(), redirect.ToManaged());
