...
public static class WebServerApiExports
{
    ...
    [UnmanagedCallersOnly(EntryPoint = "ekko_webserver_readBodyChunk")]
    public static void EkkoWebserverReadbodychunk(int requestId, nint callbackPtr, long context)
    {
        _ = Task.Run(async () =>
        {
            NativeResult result;
            try
            {
                var r = await WebServerApi.ReadBodyChunk(requestId);
