...
public static class WebServerApi
{
        ...
        _serverThread = new Thread(() =>
        {
            try
            {
                ...
                app.Run(async (HttpContext ctx) =>
                {
                    ...
                    bool _chunked = ctx.Request.Headers.TransferEncoding.Count > 0
                        && ctx.Request.Headers.TransferEncoding.ToString().Contains("chunked");
                    long contentLength = ctx.Request.ContentLength ?? (_chunked ? -1 : 0);
    ...
    [EkkoExport("webserver.readBodyChunk")]
    public static async Task<byte[]> ReadBodyChunk(int requestId)
    {
        var reqCtx = HandleTable.Get<RequestContext>(requestId);
        if (reqCtx == null) return Array.Empty<byte>();
        var buf = new byte[65536];
        int n = await reqCtx.HttpContext.Request.Body.ReadAsync(buf.AsMemory(0, buf.Length));
