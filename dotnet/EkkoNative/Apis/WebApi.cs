...
public static class WebApi
{
    ...
    private static readonly HttpClient _clientNoRedirect = new HttpClient(new SocketsHttpHandler
    {
        PooledConnectionLifetime = TimeSpan.FromMinutes(10),
    ...
    public static async Task<string> Fetch(string url, string method, string headersJson, byte[] body, string redirect)
    {
        ...
        bool noFollow = redirect == "manual" || redirect == "error";
        var res = await (noFollow ? _clientNoRedirect : _client).SendAsync(req);
        if (redirect == "error" && (int)res.StatusCode >= 300 && (int)res.StatusCode < 400)
            throw new Exception("fetch: redirected response not allowed (redirect: \"error\")");
