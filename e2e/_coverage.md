...
full recheck pass**. Bugs found via recheck: `ekko:process` (200–203, 4 FFI bugs; + m36: `child.pid`
returned the internal handle index instead of the real OS pid — `process.spawnPid` added, regression in
`process.e2e.ts`; surfaced while testing the `ekko dev` no-orphan guarantee, `system/dev-no-orphan.e2e.ts`),
`ekko:ffi` (204 —
