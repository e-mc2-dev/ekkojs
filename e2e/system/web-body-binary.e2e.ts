...
app.post("/bytes", async (req: any, res: any) => {
  const b = await req.bytes();
  let sum = 0;
  for (let i = 0; i < b.length; i++) sum += b[i];
  res.json({ len: b.length, sum });
});
app.post("/json", async (req: any, res: any) => res.json({ got: await req.json() }));
app.post("/text", async (req: any, res: any) => res.json({ got: await req.text() }));
app.start();
