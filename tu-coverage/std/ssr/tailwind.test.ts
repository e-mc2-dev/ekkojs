...
import { createApp, resolvePageAssets, cssModule } from "ekko:rune";
import { jsx as _jsx, jsxs as _jsxs, Fragment } from "ekko:jsx-runtime";
import { mkdir, writeText, remove, exists, readText } from "ekko:fs";
...
describe("Tailwind CSS support", () => {
  ...
  test("@tailwind directive detected in CSS", () => {
    ...
    try {
      const content = readText(TEST_DIR + "/globals.css");
  ...
  test("@import tailwindcss detected in CSS", () => {
    ...
    try {
      const content = readText(TEST_DIR + "/styles.css");
