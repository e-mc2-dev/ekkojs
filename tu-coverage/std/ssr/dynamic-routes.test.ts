...
describe("__matchPath with dynamic segments", () => {
  test(":param matches any value", () => {
    expect(__matchPath("/blog/:slug", "/blog/hello-world")).toBe(true);
    expect(__matchPath("/blog/:slug", "/blog/123")).toBe(true);
  });

  test(":param does not match extra segments", () => {
    expect(__matchPath("/blog/:slug", "/blog/hello/extra")).toBe(false);
  });

  test(":param does not match missing value", () => {
    expect(__matchPath("/blog/:slug", "/blog")).toBe(false);
  });

  test("static takes priority over dynamic (exact match)", () => {
    expect(__matchPath("/about", "/about")).toBe(true);
  });

  test("*param matches remaining segments", () => {
    expect(__matchPath("/docs/*path", "/docs/a")).toBe(true);
    expect(__matchPath("/docs/*path", "/docs/a/b")).toBe(true);
    expect(__matchPath("/docs/*path", "/docs/a/b/c")).toBe(true);
  });

  test("*param requires at least one segment", () => {
    expect(__matchPath("/docs/*path", "/docs")).toBe(false);
  });
});

describe("__extractParams", () => {
  test(":slug extracts single value", () => {
    const p = __extractParams("/blog/:slug", "/blog/hello-world");
    expect(p.slug).toBe("hello-world");
  });

  test("multiple :params extracted", () => {
    const p = __extractParams("/users/:id/posts/:postId", "/users/42/posts/99");
    expect(p.id).toBe("42");
    expect(p.postId).toBe("99");
  });

  test("*path extracts as array", () => {
    const p = __extractParams("/docs/*path", "/docs/a/b/c");
  ...
  test("*path with single segment", () => {
    const p = __extractParams("/docs/*path", "/docs/readme");
    expect(p.path.length).toBe(1);
    expect(p.path[0]).toBe("readme");
  });

  test("URL-decoded params", () => {
    const p = __extractParams("/blog/:slug", "/blog/hello%20world");
    expect(p.slug).toBe("hello world");
  });
});

declare function __matchPath(pattern: string, actual: string): boolean;
declare function __extractParams(pattern: string, actual: string): any;
