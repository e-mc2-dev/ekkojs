...
describe("cli basic", () => {
  ...
  test("prompt.confirm is a function", () => {
    expect(typeof prompt.confirm).toBe("function");
  });

  test("prompt.spinner is a function", () => {
    expect(typeof prompt.spinner).toBe("function");
  });

  test("prompt.spinner returns object with success and fail", () => {
    const spinner = prompt.spinner("Loading...");
