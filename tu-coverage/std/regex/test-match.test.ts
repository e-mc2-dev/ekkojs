...
describe("Regex.match", () => {
  test("returns match info for matching input", () => {
    ...
    expect(m).toBeTruthy();
    expect(m.match).toBe("alice@example.com");
    re.dispose();
  });
...
describe("Regex.matchAll", () => {
  test("returns all matches", () => {
    ...
    expect(all[0].match).toBe("1");
    expect(all[1].match).toBe("2");
    expect(all[2].match).toBe("3");
    re.dispose();
  });
  ...
  test("matchAll with email pattern", () => {
    ...
    expect(all[0].match).toBe("alice@a.com");
    expect(all[1].match).toBe("bob@b.com");
    re.dispose();
  });
});
