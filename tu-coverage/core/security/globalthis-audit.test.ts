...
describe("globalThis audit — JSX factory + Link de-globalized (import-only)", () => {
  test("Fragment not on globalThis (import from ekko:jsx-runtime)", () => {
    expect((globalThis as any).Fragment).toBeUndefined();
  });

  test("_jsx not on globalThis (import from ekko:jsx-runtime)", () => {
    expect((globalThis as any)._jsx).toBeUndefined();
  });

  test("_jsxs not on globalThis (import from ekko:jsx-runtime)", () => {
    expect((globalThis as any)._jsxs).toBeUndefined();
  });

  test("Link not on globalThis (rune Link is an import)", () => {
    expect((globalThis as any).Link).toBeUndefined();
  });
});

describe("globalThis audit — React hooks present as globals", () => {
...
describe("globalThis audit — internal bridges removed from global", () => {
  test("__SpawnError not on globalThis (internal, scoped)", () => {
    expect((globalThis as any).__SpawnError).toBeUndefined();
  });

  test("__channelAsyncIterator not on globalThis (internal, scoped)", () => {
    expect((globalThis as any).__channelAsyncIterator).toBeUndefined();
  });

  test("__mimir not on globalThis", () => {
    expect((globalThis as any).__mimir).toBeUndefined();
  });

  test("AssertionError not on globalThis (import from ekko:test/assert)", () => {
    expect((globalThis as any).AssertionError).toBeUndefined();
  });
});
...
describe("globalThis audit — enumeration sweep", () => {
  ...
  const TEST_HARNESS_GLOBALS = ["__ekko_mock_module", "__ekko_unmock_module"];

  test("Object.keys(globalThis) contains no __ entries beyond the test-mode mock API", () => {
    const leaked = Object.keys(globalThis)
      .filter((k: string) => k.startsWith("__") && !TEST_HARNESS_GLOBALS.includes(k));
    expect(leaked).toHaveLength(0);
  });

  test("for..in globalThis finds no __ entries beyond the test-mode mock API", () => {
    const leaked: string[] = [];
    for (const k in globalThis) {
      if (k.startsWith("__") && !TEST_HARNESS_GLOBALS.includes(k)) leaked.push(k);
    }
    expect(leaked).toHaveLength(0);
  });
...
describe("regression — React hooks functional", () => {
...
describe("regression — request body parsing", () => {
  ...
  test("req.json() parses JSON body", async () => {
    ...
    server.post("/echo-json", async (req: any, res: any) => {
